 ...
 *
 * Active-Set 알고리즘은 부등식 제약 중 현재 해(u)에서 정확히 경계면에 도달한 제약(활성화된 제약,
 * Active Constraint)을 등식 제약처럼 취급(Working Set)하여 하위 문제(KKT 시스템)를 풀고, 라그랑주
 * 승수(Lagrange Multiplier)와 탐색 방향(Search Direction)을 평가하여 제약을 추가(Add)하거나
 * 제거(Drop)하는 과정을 반복하며 최적해를 찾는 기법입니다.
 *
...
class ActiveSetSolver {
   public:
    ...
    StaticMatrix<double, N_vars, N_vars>
        P;  ///< 목적 함수의 2차항 계수 행렬 (Hessian). 양의 정부호(Positive Definite)여야 함.
   ...
   private:
     ...
     * 부등식 제약이 모두 활성화될 수 있는 최악의 경우(Worst-case)를 대비하여 정적 메모리를 미리
     * 할당합니다.
     */
    static constexpr size_t KKT_Size = N_vars + N_eq + N_ineq;

    StaticMatrix<double, KKT_Size, KKT_Size>
        KKT_Matrix;  ///< 매 반복마다 구성되는 KKT 시스템 좌변 행렬
    StaticVector<double, KKT_Size> KKT_rhs;  ///< 매 반복마다 구성되는 KKT 시스템 우변 벡터 (잔차)
    StaticVector<double, KKT_Size>
        KKT_solution;  ///< 풀이 결과 (탐색 방향 p 및 제약 조건의 라그랑주 승수 λ)
   ...
   public:
    ...
    void build_masked_KKT(const StaticVector<double, N_vars>& u_k) {
        ...
        if constexpr (N_ineq > 0) {
            ...
            for (size_t i = 0; i < N_ineq; ++i) {
                if (working_set[i]) {
                    // [활성화 됨] : 현재 해가 부등식 경계에 도달해 있으므로 등식 제약처럼 시스템에
                    // 삽입
                    ...
                    // KKT 행렬의 크기를 동적으로 줄이지 않고 대각 원소를 1.0으로, 우변을 0.0으로
                    // 두어 해당 라그랑주 승수가 0이 되도록 유도하고 행렬이 가역행렬(Invertible)이
                    // 되도록 보장함.
     ...
     *                    반드시 모든 부등식 제약을 만족하는 Feasible한 초기값이어야 합니다 (Primal
     * 방식의 전제조건).
     * @param max_iter 무한 루프 방지용 최대 반복 횟수 (제약 조건의 추가/제거 반복을 통제)
     * @return bool 최적해 수렴에 성공하면 true, KKT 행렬 분해 실패 혹은 최대 반복 초과 시 false
     * 반환
     */
    bool solve(StaticVector<double, N_vars>& u, int max_iter = 100) {
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            // -------------------------------------------------------------
            // 탐색 방향이 거의 0이라는 것은 현재의 Working Set 하에서는 더 이상 갈 곳이 없음을
            // 의미합니다.
            if (p_norm < 1e-6) {
                ...
                if (drop_idx == -1) {
                    ...
                    // 현재 해가 전역적인 최적점(Global Optimum)이며 KKT 조건을 완벽히 충족한
                    // 것입니다.
                    return true;
                } else {
            ...
            // -------------------------------------------------------------
                ...
                // p 방향으로 이동했을 때의 각 부등식 제약 조건의 변화량(a_p)과 현재 제약 조건
                // 값(a_u)
                StaticVector<double, N_ineq> a_p = A_ineq * p;
                StaticVector<double, N_ineq> a_u = A_ineq * u;

                for (size_t i = 0; i < N_ineq; ++i) {
                    // 비활성화된 제약 중, 탐색 방향 p가 제약 경계면을 향해 가고 있을 때만 (a_p_i >
                    // 0) 검사합니다. a_p_i <= 0 인 경우는 제약 조건 안쪽(더 안전한 방향)으로 가고
                    // 있으므로 충돌할 일이 없습니다.
                    if (!working_set[i] && a_p(static_cast<int>(i)) > 1e-8) {
                        ...
                        double alpha_i = (b_ineq(static_cast<int>(i)) - a_u(static_cast<int>(i))) /
                                         a_p(static_cast<int>(i));

                        // 현재까지 찾은 가장 짧은 거리보다 짧다면 갱신 (가장 먼저 부딪히는 경계면
                        // 찾기)
                        if (alpha_i < alpha) {
                            alpha = alpha_i;
                ...
                for (size_t i = 0; i < N_vars; ++i) {
                    u(static_cast<int>(i)) =
                        u(static_cast<int>(i)) + (p(static_cast<int>(i)) * alpha);
                }

                // 스텝 사이즈가 1.0 미만으로 잘렸다는 것은 이동 중에 새로운 부등식 경계면과
                // 부딪혔음을 의미합니다. 부딪힌 제약 조건을 Working Set에 추가(Add)하여, 다음
                // 반복부터는 이 제약을 넘어서 밖으로 나가지 못하도록 등식 제약처럼 고정시킵니다.
                if (blocking_idx != -1) {
        ...
        // 흔한 실패 원인으로는 제약 조건이 상충(Infeasible)하여 Cycling(Add/Drop 반복 무한루프)이
        // 발생한 경우가 있습니다.
        return false;
    }
};
