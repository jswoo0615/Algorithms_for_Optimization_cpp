                ...
                for (size_t i = 0; i < N_ineq; ++i) {
                    if (working_set[i]) {
                        ...
                        for (size_t j = 0; j < N_vars; ++j) {
                            KKT_Matrix(static_cast<int>(ineq_offset + i), static_cast<int>(j)) = A_ineq(static_cast<int>(i), static_cast<int>(j));
                            KKT_Matrix(static_cast<int>(j), static_cast<int>(ineq_offset + i)) = A_ineq(static_cast<int>(i), static_cast<int>(j));
                        }

                        // RHS 잔차 : 활성화된 제약의 거리 b_ineq - A_ineq * u_k
                        double ineq_val = 0;
                        for (size_t j = 0; j < N_vars; ++j) {
                            ineq_val += A_ineq(static_cast<int>(i), static_cast<int>(j)) * u_k(static_cast<int>(j));
                        }
                        KKT_rhs(static_cast<int>(ineq_offset + i)) = b_ineq(static_cast<int>(i)) - ineq_val;
                    } else {
                        // 비활성화 됨 : 역행렬 특이성 (Singular)을 방지하기 위해 대각 원소 1.0 삽입
                        KKT_Matrix(static_cast<int>(ineq_offset + i), static_cast<int>(ineq_offset + i)) = 1.0;
