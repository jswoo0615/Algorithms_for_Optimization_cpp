 ...
 * 분모가 무한히 커지고, 결과적으로 학습률(Step Size)이 0에 수렴하여 학습이 조기 종료(Premature
 * Convergence)되는 단점이 있습니다.
 *
 * AdaDelta는 이를 극복하기 위해 제안된 알고리즘으로, 두 가지 핵심 개선점을 가집니다:
 * 1. 전체 과거 기울기가 아닌, 최근 기울기 정보에 가중치를 두는 '지수 이동 평균(Exponential Moving
 * Average, EMA)'을 사용하여 기울기 제곱을 추적합니다.
 * 2. 변수 업데이트량(Δx)의 지수 이동 평균도 함께 추적합니다. 이를 통해 업데이트의 물리적
 * 단위(Unit)를 맞추어 사용자가 직접 지정해야 하는 글로벌 '학습률(Learning Rate)' 하이퍼파라미터를
 * 완전히 제거했습니다.
 *
 * @note 본 클래스는 MISRA C++ 코딩 표준을 지향하여 동적 메모리 할당을 배제하고, O(1) 정적 메모리와
 * FMA(Fused Multiply-Add) 명령어 지원, SIMD 벡터 최적화 구문을 통해 성능을 극대화하도록
 * 설계되었습니다.
 */
class AdaDelta {
   public:
     ...
     * @param gamma 감쇠율 (Decay Rate, 보통 0.9 ~ 0.99 사용). 과거 정보가 잊혀지는 속도를 결정하는
     * 가중치 파라미터.
     * @param epsilon 0으로 나누어지는 것(Division by zero)을 방지하기 위한 수치적 안정성 상수 (보통
     * 1e-8 등 매우 작은 양수)
     * @param max_iter 무한 루프를 방지하기 위한 최대 허용 반복 횟수
     * @param tol 수렴 판정을 위한 허용 오차. 기울기(Gradient) 벡터의 L2-Norm(크기)이 이 값보다
     * 작아지면 극값(최적점)에 도달했다고 판정합니다.
     * @param verbose 콘솔에 최적화 진행 상황(반복 횟수, 함수값, 기울기 크기 등)을 출력할지 여부
     * @return OptimizationResultND<N> 최적화가 완료된 해(x_opt), 최소 목적 함수 값(f_opt), 실제
     * 수행된 반복 횟수, 소요 시간 정보
     */
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> x_init,
        ...
        // 캐시 라인 정렬(64바이트)을 통해 SIMD 벡터 연산 및 캐시 히트 성능을 극대화한 변수 배열
        // 선언
        ...
        // 분자에 위치하며, 글로벌 학습률을 대체하여 파라미터 업데이트 시 단위(Unit)의 일관성을
        // 맞추는 핵심 요소입니다.
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // 정적 배열을 활용한 AutoDiff 기능으로 동적 메모리 할당 없이 O(1)의 안정성을
            // 보장합니다.
            AutoDiff::value_and_gradient<N>(f, x, f_x, g);

            // 현재 위치에서 기울기 벡터의 제곱합(L2-Norm의 제곱)을 계산합니다.
            double g_norm_sq = 0.0;
#pragma omp simd  // 컴파일러에게 이 루프를 SIMD(Single Instruction Multiple Data) 병렬 명령어로
                  // 처리하도록 지시
            for (size_t i = 0; i < N; ++i) {
                // std::fma(a, b, c) = (a * b) + c
                // 단일 사이클 하드웨어 연산으로 처리하여 정밀도를 높이고 부동소수점 반올림 오차를
                // 줄입니다.
...
#pragma omp simd  // 벡터화(Vectorization) 강제 지시어
            for (size_t i = 0; i < N; ++i) {
                ...
                // 이전까지의 누적 정보 s[i]에 gamma를 곱하여 서서히 잊게 만들고, 현재 기울기의 제곱
                // 비중을 (1-gamma)만큼 반영합니다.
