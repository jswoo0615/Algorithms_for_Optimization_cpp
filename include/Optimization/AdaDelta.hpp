 ...
 * 1. 기울기 누적합 대신 지수 이동 평균(Exponential Moving Average, EMA)을 사용하여 최근 기울기
 * 정보에 가중치를 부여합니다.
 * 2. 변수 업데이트량(Δx)의 지수 이동 평균을 함께 추적하여, 사용자가 직접 지정해야 하는 글로벌
 * '학습률(Learning Rate)' 파라미터를 완전히 제거했습니다. (업데이트의 물리적 단위가 일치하도록 차원
 * 분석을 통해 자체적인 Step Size를 생성합니다.)
 *
 * @note MISRA C++ 코딩 표준을 지향하여 동적 메모리 할당을 배제하고, O(1) 정적 메모리와 FMA(Fused
 * Multiply-Add) 명령어를 통해 최적화 속도를 가속합니다.
 */
class AdaDelta {
   public:
     ...
     * @param gamma 감쇠율 (Decay Rate, 보통 0.9 ~ 0.99 사용). 과거 정보가 잊혀지는 속도를
     * 결정합니다.
     * @param epsilon 0으로 나누어지는 것을 방지하는 수치적 안정성 상수 (작은 양수)
     * @param max_iter 무한 루프를 방지하기 위한 최대 허용 반복 횟수
     * @param tol 수렴 판정을 위한 허용 오차 (Gradient의 노름(Norm)이 이 값보다 작아지면 수렴으로
     * 판정)
...
#pragma omp simd  // 컴파일러에게 SIMD 병렬 처리를 지시
            for (size_t i = 0; i < N; ++i) {
                // std::fma(a, b, c) = a * b + c 연산을 하드웨어 단에서 한 번에 처리하여 부동소수점
                // 오차 축소 및 속도 향상
...
#pragma omp simd
            for (size_t i = 0; i < N; ++i) {
                ...
                // 이전 반복까지의 누적값(s[i])에 감쇠율(gamma)을 곱하고, 현재 기울기 제곱에 (1 -
                // gamma)를 곱해 더함.
                ...
                // 학습률 대신 (rms_u / rms_s) 비율을 사용하여 각 파라미터별로 맞춤형 스텝 폭을
                // 결정합니다.
