   ...
   */
   class AdaGrad {
    public:
      ...
      [[nodiscard]] static std::array<double, N> optimize(Func f, std::array<double, N> x, double alpha = 0.1,
                                                          double epsilon = 1e-8, size_t max_iter = 15000, double tol=1e-4,
                                                          bool verbose = false) {
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
          ...
          if (g_norm < epsilon) {
            if (verbose) {
              std::cout << "✅ Convergence achieved at iteration " << iter << " with gradient norm " << g_norm << ".\n";
            }
            break;
          }
