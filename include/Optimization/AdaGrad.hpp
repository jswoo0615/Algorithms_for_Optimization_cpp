 ...
 * 동일한 학습률(Learning Rate)을 적용하는 대신, **각 매개변수마다 맞춤형 학습률을 계산**하여
 * 적용하는 알고리즘입니다.
 *
 * 핵심 아이디어:
 * 과거에 많이 변화했던(기울기가 컸던) 변수는 학습률을 작게 하여 세밀하게 조정하고,
 * 과거에 거의 변화하지 않았던(기울기가 작았던) 변수는 학습률을 크게 하여 빠르게 학습되도록
 * 유도합니다. 이는 희소한(Sparse) 특징을 가진 데이터나 함수에서 특히 효과적입니다.
 *
 * 단점:
 * 과거 기울기의 제곱이 계속 누적만 되기 때문에($G_i$ 가 단조 증가),
 * 반복 횟수가 많아질수록 학습률이 $0$에 수렴하여 결국 학습이 멈추는(Premature Convergence) 문제가
 * 발생할 수 있습니다. (이를 해결하기 위해 등장한 것이 RMSProp, AdaDelta 등입니다.)
 *
 * @note MISRA C++ 및 실시간 제어(RT) 코딩 표준 준수:
 *       동적 메모리 할당(힙 할당)을 완전히 배제하고 O(1) 정적 배열 및 In-place 연산을 사용하여
 * 속도와 안정성을 극대화했습니다.
 */
class AdaGrad {
   public:
     ...
     * @param epsilon 0으로 나누어지는 것을 방지하기 위한 수치적 안정성 상수 (작은 양수, 기본값:
     * 1e-8)
     * @param max_iter 무한 루프를 방지하기 위한 최대 허용 반복 횟수
     * @param tol 수렴 판정을 위한 허용 오차 (현재는 수렴 판정에 epsilon 파라미터를 그대로
     * 재사용하고 있습니다)
    ...
    [[nodiscard]] static std::array<double, N> optimize(Func f, std::array<double, N> x,
        ...
        // G: 각 변수(차원)별로 과거 기울기 제곱을 누적하여 저장하는 배열 ($G_i = \sum_{t=1}^k
        // g_{i,t}^2$) 동적 할당이 없는 스택(Stack) 메모리인 std::array를 사용하여 실시간성을
        // 보장합니다. 제어 이론의 관례에 따라 속도(Velocity, v) 모멘텀과 혼동되지 않도록 변수명 G를
        // 사용합니다.
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            for (size_t i = 0; i < N; ++i) {
                ...
                // [수치적 안정성] epsilon을 \sqrt{G_i} 바깥에 더하여 분모가 0이 되어
                // 발산(Divergence)하는 것을 방지합니다. [실시간성/최적화] '-=' In-place 연산자를
                // 사용하여 캐시 메모리 히트율을 극대화합니다.
