   ...
   */
   class Adam {
    public:
      ...
      [[nodiscard]] static std::array<double, N> optimize(Func f, std::array<double, N> x, double alpha=0.1, double beta1=0.9, double beta2=0.999,
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
          ...
          if (g_norm_sq < tol_sq) {
            if (verbose) {
                std::cout << "✅ Convergence achieved at iteration " << iter << " (f(x) = " << f_x << ", ||g||^2 = " << g_norm_sq << ")\n";
            }
            break;
          }
