 ...
 * 
 * Adam은 모멘텀(Momentum) 최적화 방식과 RMSProp 알고리즘의 장점을 결합하여 만든 가장 대중적이고 강력한 최적화 기법 중 하나입니다.
 * 각 파라미터(변수)마다 1차 모멘트(Gradient의 지수 이동 평균)와 2차 모멘트(Gradient 제곱의 지수 이동 평균)를 모두 계산하여,
 * 학습 방향과 스텝 크기(학습률)를 동시에 동적으로 조절합니다.
 * 
 * 핵심 아이디어:
 * 1. **1차 모멘트 (m)**: 과거 기울기의 방향(관성)을 유지하여, 진동을 줄이고 빠르게 수렴하도록 돕습니다. (Momentum의 역할)
 * 2. **2차 모멘트 (v)**: 과거 기울기 크기의 제곱을 추적하여, 자주 변하는 파라미터는 세밀하게(작은 스텝), 거의 변하지 않는 파라미터는 큼직하게(큰 스텝) 이동시킵니다. (RMSProp의 역할)
 * 3. **편향 보정 (Bias Correction)**: 초기 반복 시 m과 v가 0으로 편향(Bias)되는 것을 수학적으로 보정하여 초기부터 올바른 방향을 잡도록 돕습니다.
 * 
 * @note 실시간(RT) 환경과 최적화 성능을 위해 값비싼 `std::pow()` 함수 호출을 배제하고, 반복 시마다 `beta`를 곱해나가는 누적 곱(Running Product) 기법을 사용했습니다.
 */
class Adam {
   public:
     ...
     * @param beta2 2차 모멘트(기울기 제곱)에 대한 지수 감쇠율 (보통 1차 모멘트보다 0.999 등으로 1에 더 가깝게 설정합니다)
     * @param epsilon 0으로 나누어지는 것을 방지하기 위한 수치적 안정성 상수 (기본값: 1e-8)
     * @param max_iter 무한 루프를 방지하기 위한 최대 허용 반복 횟수
     * @param tol 수렴 판정을 위한 허용 오차 (Gradient의 Norm 제곱이 이 값보다 작아지면 수렴으로 판정)
    ...
    [[nodiscard]] static std::array<double, N> optimize(Func f, std::array<double, N> x,
        ...
        // [성능 최적화] 매 이터레이션(루프)마다 g_norm 계산 시 std::sqrt()를 호출하는 것을 막기 위해
        // 허용 오차(tol)를 미리 제곱해 둡니다.
        const double tol_sq = tol * tol;

        // 편향 보정에 사용될 beta1^t 와 beta2^t 의 누적값
        // std::pow(beta1, iter) 를 매번 호출하는 대신, 매 반복마다 변수에 beta1, beta2를 곱해줍니다.
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            for (size_t i = 0; i < N; ++i) {
                ...
                // 초기화 시점(t=0)에 m과 v가 0으로 초기화되어 있으므로, 초기 몇 번의 반복 동안은 0에 가깝게 편향됩니다.
                // 이를 방지하기 위해 (1 - beta^t) 로 나누어 스케일을 원복시킵니다.
