...
/**
 * @brief 정적 메모리 엔진(Layer 1)과 완벽히 호환되는 Forward-Mode Automatic Differentiation (자동
 * 미분) 클래스
 *
 * 복잡한 수학적 함수의 도함수(기울기, 야코비안 등)를 해석적으로 구하지 않고,
 * 이원수(Dual Numbers) 대수학을 활용하여 기계적으로 정확하게 계산(컴퓨터의 소수점 표현 한계인 절단
 * 오차 없이 계산)하는 기법입니다. 유한 차분법(Finite Difference)에서 발생하는 수치적 근사
 * 오차(Truncation Error)가 발생하지 않아 최적화 알고리즘(예: SQP, Newton's Method)에서 매우
 * 안정적인 미분값을 제공합니다.
 *
 * @note std::array를 전면 배제하고, 수학적 행렬 연산에 최적화된 내부 모듈인 StaticVector /
 * StaticMatrix를 사용합니다.
 * @note SQP의 BFGS 업데이트를 위해 수치적 오차가 큰 유한 차분법(Finite Difference) 기반의 Hessian은
 * 삭제하였습니다.
 */
class AutoDiff {
   public:
     ...
     * 이를 위해 N개의 차원에 대해 N번의 함수 평가를 수행하며, 매 반복마다 이원수(Dual Number)를
     * 이용해 한 방향의 편미분 계수(Seed 주입)를 정확하게 추출합니다.
     *
     * @tparam N 입력 벡터의 차원
    ...
    [[nodiscard]] static StaticVector<double, N> gradient(Func f,
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            for (size_t j = 0; j < N; ++j) {
                // i번째 변수에 대해서만 미분값(d, Dual Number의 쌍대 부분)을 1.0으로 부여하여
                // "Seed"를 주입합니다. 이는 수식 상 편미분 ∂f / ∂x_i 를 구하기 위한 설정입니다. j
                // != i 인 다른 변수들의 미분값은 0.0으로 두어 상수로 취급합니다.
    ...
    [[nodiscard]] static StaticMatrix<double, M, N> jacobian(Func f,
        ...
        // Reverse-Mode AD보다 계산 비용이 더 클 수 있으나, 소규모 최적화 문제에서는 구현이 단순하고
        // 매우 효율적입니다.
        for (size_t i = 0; i < N; ++i) {
            StaticVector<Dual<double>, N> x_dual;
            for (size_t j = 0; j < N; ++j) {
            ...
            // 이는 각 출력 함수 f_k 가 i번째 입력 변수 x_i 의 미세한 변화에 어떻게 반응하는지를
            // 의미합니다. 따라서 각 출력 차원 k에 대하여, i번째 열에 편미분 값을 채워 넣습니다.
