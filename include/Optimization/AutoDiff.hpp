...
public:
    ...
    template <size_t N, typename Func>
    [[nodiscard]] static StaticVector<double, N> gradient(Func f, const StaticVector<double, N>& x) {
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            for (size_t j = 0; j < N; ++j) {
                ...
                x_dual(static_cast<int>(j)) = Dual<double>(x(static_cast<int>(j)), (i == j) ? 1.0 : 0.0);
            }
    ...
    // ============================================================
    template <size_t N, typename Func>
    static void value_and_gradient(Func f, const StaticVector<double, N>& x,
                                   double& v_out, StaticVector<double, N>& g_out) {
        g_out.set_zero();
    ...
    [[nodiscard]] static StaticMatrix<double, M, N> jacobian(Func f, const StaticVector<double, N>& x) {
