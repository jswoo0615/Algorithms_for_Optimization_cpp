...
namespace Optimization {
    ...
    class AutoDiff {
        public:
            ...
            [[nodiscard]] static constexpr double value(Func f, const std::array<double, N>& x_point) noexcept {
                return f(x_point);
            }
            ...
            // ============================================================
            template <size_t N, typename Func>
            [[nodiscard]] static constexpr std::array<double, N> gradient(Func f, const std::array<double, N>& x_point) noexcept {
            ...
            [[nodiscard]] static constexpr std::array<std::array<double, N>, M> jacobian(Func f, const std::array<double, N>& x_point) noexcept {
                std::array<DualVec<double, N>, N> x_dual{};
                for (size_t i = 0; i < N; ++i) {
                    x_dual[i] = DualVec<double, N>::make_variable(x_point[i], i);
                } 
            ...
            // ============================================================
            template <size_t N, typename Func>
            [[nodiscard]] static std::array<std::array<double, N>, N> hessian(Func f, const std::array<double, N>& x) noexcept {
