 ...
 * 순수 뉴턴법(Newton's Method)은 매 반복마다 값비싼 헤시안(2차 미분 행렬)의 계산과 그 역행렬을 직접 구해야 하는 단점이 있습니다.
 * 
 * 반면 BFGS 알고리즘은 1차 미분(Gradient) 정보만을 누적하여 **역헤시안(Inverse Hessian) 행렬을 근사**해 나갑니다.
 * 이로 인해 역행렬을 구하는 과정 없이(O(N^2) 연산만으로) 뉴턴법에 가까운 2차 수렴 속도(Quadratic Convergence)를 달성합니다.
 * 
...
#pragma omp simd
            ...
            // y^T * s > 0 을 만족해야만 V 행렬이 양의 정부호(Positive Definite) 성질을 유지할 수 있습니다.
            if (y_dot_s > 1e-10) {
                ...
                // 행렬이 무너지는 것을 막기 위해 V를 안전한 초기 상태(단위 행렬)로 리셋합니다 (Restart).
                for (size_t i = 0; i < N; ++i) {
                    for (size_t j = 0; j < N; ++j) {
                        V[i][j] = (i == j) ? 1.0 : 0.0;
                    }
