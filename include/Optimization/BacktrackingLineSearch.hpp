 ...
 *
 * 최적화 과정에서 주어진 탐색 방향(d)으로 얼마나 멀리 이동할지(스텝 사이즈, alpha)를 결정하는 근사
 * 선 탐색(Inexact Line Search) 기법입니다. 함수 값이 충분히 감소했는지를 판단하는
 * **아르미호(Armijo) 충분 감소 조건**을 만족할 때까지 스텝 사이즈를 일정 비율(p)로
 * 줄여나갑니다(Backtracking).
 *
 * 특징:
 * 1. 정확한 최소점을 찾는 Exact Line Search 보다 계산 비용이 훨씬 저렴하며, 실전에서 널리 쓰입니다.
 * 2. 초기 스텝 사이즈(alpha=1.0)부터 시작하여 조건을 만족하지 못하면 뒤로 물러나므로
 * "Backtracking"이라 부릅니다.
...
#pragma omp simd
   ...
   public:
     ...
     * @param c Armijo 조건의 엄격함을 결정하는 상수 (보통 1e-4 처럼 매우 작은 양수 사용. 작을수록
     * 쉽게 승인됨)
...
#pragma omp simd
        ...
        // 방향 도함수가 0 이상이라는 것은 현재 방향(d)이 목적 함수 값을 줄이는 '하강 방향(Descent
        // Direction)'이 아님을 의미합니다.
        ...
        while (true) {
            ...
            // 1차 테일러 전개 기반의 예측 감소량에 상수 c를 곱한 만큼은 '최소한' 감소해야 한다고
            // 엄격하게 제한합니다.
            ...
            // Armijo 조건 검사 : 함수 값이 목표치보다 작거나 같으면 승인(Accept)하고 스텝 사이즈
            // 확정
            if (f_new <= target_val) {
    ...
    /**
     * @brief 함수값과 기울기를 함수 내부에서 자동으로 계산한 후 선 탐색을 수행하는 편의성(Overload)
     * 함수입니다.
     *
     * @tparam N 차원 수
     * @tparam Func 목적 함수 타입
