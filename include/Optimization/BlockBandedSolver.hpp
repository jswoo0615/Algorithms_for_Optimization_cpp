...
/**
 * @brief NMPC(Nonlinear Model Predictive Control) 최적화를 위한 블록 밴드형(Block-Banded)
 * 리카티(Riccati) 솔버 클래스.
 *
 ...
 *
 * 이 솔버는 동적 메모리 할당(힙 할당)을 배제하여, 임베디드 실시간 제어(Hard Real-Time Control)에
 * 적합합니다.
 *
...
class BlockBandedSolver {
   public:
    /**
     * @brief 호라이즌 전체의 시점(t = 0 ~ Np-1)마다 필요한 선형화 모델 및 비용 함수 데이터를 담는
     * 배열. 동적 할당 없이 std::array를 사용하여 스택(또는 BSS 섹션)에 할당됩니다.
     */
    ...
    /** @brief 0부터 Np까지 각 시점의 상태 궤적 오차(State trajectory error, Delta x). 크기는 Np + 1
     * 입니다. */
    std::array<StaticVector<double, Nx>, Np + 1> dx;

    /** @brief 0부터 Np-1까지 각 시점의 최적 제어 입력 오차(Control input error, Delta u). 크기는 Np
     * 입니다. */
     ...
     *
     * 목적지 (t = Np)에서 시작하여 현재 시점 (t = 0)까지 시간을 역행하며 가치 함수(Value
     * Function)의 2차 미분(Hessian) 행렬인 `P`와 1차 미분(Gradient) 벡터인 `p_vec`를 갱신합니다. 이
     * 과정을 통해 각 시점 k에서 최적의 피드백 제어 게인(K_k)과 피드포워드 제어 게인(k_ff_k)을
     * 도출합니다.
     *
     * @return bool 연산 중 H_uu 행렬의 역행렬을 구할 수 없는 상태(수치적 불안정성, Positive
     * Definite 조건 위배)가 발생하여 실패하면 false, 정상적으로 역방향 탐색을 마치면 true를
     * 반환합니다.
     */
    bool backward_pass() {
        // 1. 종단 조건 (Terminal Condition)을 초기 가치 함수의 파라미터로 설정 (V_N(x) = Terminal
        // Cost)
        ...
        for (int k = static_cast<int>(Np) - 1; k >= 0; --k) {
            ...
            // [Step 2-1] Q 함수 (State-Action Value Function) 근사를 위한 블록 텐서(Hessian 블록)
            // 계산 H_xx = Q_k + A_k^T * P_{k+1} * A_k (상태에 대한 2차 미분)
            ...
            if (!H_uu.LDLT_decompose()) {
                // H_uu가 양의 정부호(Positive Definite)가 아니어 역행렬을 구할 수 없는 경우 (수치적
                // 불안정성 발생) 대각 원소에 작은 양수(1e-4)를 더해주는 Damping(Tikhonov
                // Regularization)을 적용하여 방어합니다.
     ...
     * @return bool 성공적으로 해를 찾았으면 true, 역행렬 계산 불가 등 수치적 오류로 실패했으면
     * false 반환
     */
    bool solve(const StaticVector<double, Nx>& dx_0) {
        ...
        // 2. 도출된 제어 이득을 바탕으로 초기 상태 dx_0부터 순방향으로 제어 입력과 상태 궤적
        // 시뮬레이션
        forward_pass(dx_0);
