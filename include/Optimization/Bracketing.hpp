 ...
 * 이를 통해 극솟값이 확실히 포함된 세 점(a, b, c)을 찾아내고, 최종적으로 그 구간(Range)을
 * 반환합니다.
 *
 ...
 * @param k 스텝 확장 배율 (기본값: 2.0). 한 번 탐색할 때마다 다음 스텝의 크기를 k배만큼 늘려 빠른
 * 구간 탐색을 유도합니다.
...
inline Range bracket_minimum(Func f, double x, double s = 0.01, double k = 2.0) {
    ...
    while (true) {
        ...
        // 하강하던 추세가 꺾이고 다시 상승한다는 의미이므로 구간(a, c) 사이에 극솟값이 존재하게
        // 됩니다.
        if (yc > yb) {
...
inline double bisection_method(FuncDual f, Range range, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // 실수부(Real)에는 위치 'mid'를, 쌍대부(Dual)에는 미분 계수 시딩(Seeding)을 위한 '1.0'을
        // 넣습니다.
        auto res = f(Dual<double>(mid, 1.0));

        // [구간 축소 로직]
        // res.d 는 이원수 연산을 통해 자동으로 계산된 현재 위치(mid)에서의 1차 미분값(기울기,
        // f'(mid))입니다.
        if (res.d > 0) {
            ...
            // 기울기가 음수(-)라는 것은 현재 지점 기준 왼쪽은 함수값이 증가(또는 감소 중)한다는
            // 뜻입니다. 극솟값은 오른쪽 구간 [mid, b]에 존재하므로 왼쪽 경계(a)를 당깁니다.
            a = mid;
        }
    }
    // 탐색이 완료된 후 최종적으로 축소된 구간의 중앙값을 최적점으로 반환합니다.
 ...
 * 함수의 기울기가 L을 넘지 않는다는 성질을 이용하여, 이미 평가한 점들을 바탕으로 함수값의
 * '하한선(Lower Bound)'을 V자 톱날(Sawtooth) 모양으로 그려가며, 그 하한선이 가장 깊게 파인 곳을
 * 다음 탐색 지점으로 선정합니다.
 */
struct Pt {
    double x;  ///< X 좌표 (위치)
 ...
 * @param eps 수렴 판정을 위한 허용 오차. 하한선(best_z)과 현재 최소 함수값(min_f)의 차이가 이보다
 * 작으면 종료.
    ...
    for (int i = 0; i < max_iter; ++i) {
        ...
        for (size_t j = 0; j < pts.size() - 1; ++j) {
            ...
            if (z < best_z) {
                best_z = z;
                best_idx =
                    static_cast<int>(j);  // 가장 유망한(낮은) 교점이 속한 구간의 왼쪽 인덱스 저장
            }
        }
