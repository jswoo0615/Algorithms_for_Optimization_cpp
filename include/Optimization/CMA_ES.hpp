 ...
 * @note 목적 함수의 역-헤시안(Inverse Hessian)을 학습하듯 공분산 행렬을 적응시키는 전역 최적화 기법.
 * 정적 메모리(O(1)) 기반의 촐레스키 분해(Cholesky Decomposition) 엔진 내장.
 */
class CMA_ES {
private:
    template <size_t N>
...
public:
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(
        Func f, std::array<double, N> mu_init,
        double sigma = 1.0, size_t max_iter = 1000, 
        double tol = 1e-6, uint32_t seed = 12345, bool verbose = false) noexcept 
    {
        ...
        const double c_mu = std::min(1.0 - c_1, 2.0 * (mu_eff - 2.0 + 1.0 / mu_eff) / ((N + 2.0)*(N + 2.0) + mu_eff));
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            double h_sigma = (norm_p_sigma / std::sqrt(1.0 - std::pow(1.0 - c_sigma, 2.0 * iter)) 
                              < (1.4 + 2.0 / (N + 1.0)) * E_norm_N) ? 1.0 : 0.0;

            #pragma omp simd
            for (size_t i = 0; i < N; ++i) {
                p_cov[i] = std::fma(1.0 - c_cov, p_cov[i], 
            ...
            for (size_t i = 0; i < N; ++i) {
                for (size_t j = i; j < N; ++j) { // 대칭 행렬이므로 절반만 연산
                    ...
                    double rank_1_update = p_cov[i] * p_cov[j] + (1.0 - h_sigma) * c_cov * (2.0 - c_cov) * C[i][j];
                    C[i][j] = (1.0 - c_1 - c_mu) * C[i][j] + c_1 * rank_1_update + c_mu * rank_mu_update;
                    C[j][i] = C[i][j]; // 대칭성 강제
                }
            }
            ...
            if (verbose && (iter % 10 == 0 || iter == 1)) {
                std::cout << "[Iter " << std::setw(3) << iter 
                          << "] Best f(x): " << std::fixed << std::setprecision(5) << pop[0].y 
                          << " | sigma: " << sigma << "\n";
            }
        }
