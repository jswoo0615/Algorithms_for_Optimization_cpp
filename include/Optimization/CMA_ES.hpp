 ...
 *
 * @note 동적 할당(Heap Allocation)을 철저히 배제하고 정적 배열(`std::array`)만을 사용하여
 * 구현되었으며, 내부적으로 O(1) 메모리를 사용하는 촐레스키 분해(Cholesky Decomposition) 엔진을
 * 내장하고 있습니다.
 */
class CMA_ES {
   private:
   ...
   public:
     ...
     * @param sigma 초기 탐색 보폭(Step Size / Standard Deviation). 문제의 스케일에 맞게 설정해야
     * 함.
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> mu_init,
        ...
        alignas(64) std::array<double, N> p_sigma = {
            0.0};  // 보폭 조절을 위한 진화 경로 (켤레 방향 누적)
...
#pragma omp simd
            ...
            alignas(64) std::array<double, N> step_x = {
                0.0};  // 실제 스케일 공간에서의 이동량 (sigma 제외)
...
#pragma omp simd
            ...
            // 연속적인 탐색 방향을 갖는지(경로가 길어지는지) 혹은 방향이 번갈아 바뀌는지(경로가
            // 상쇄되는지) 기억합니다.
...
#pragma omp simd
            ...
            // Heaviside step function: 진화 경로가 너무 길어질 때 공분산 행렬 업데이트가 발산하는
            // 것을 막는 스위치
...
#pragma omp simd
            ...
            for (size_t i = 0; i < N; ++i) {
                for (size_t j = i; j < N;
                     ++j) {  // 대칭 행렬(Symmetric)이므로 위쪽 삼각 부분(i <= j)만 계산
                    // [Rank-mu 업데이트 항 계산]
            ...
            // 진화 경로 p_sigma의 길이가 기댓값(E_norm_N)보다 길면 연속적인 하강 방향이라는
            // 뜻이므로 보폭을 늘리고(sigma 증가), 기댓값보다 짧으면 진동하고 있다는 뜻이므로 보폭을
            // 줄입니다(sigma 감소).
