 ...
 * 
 * 1차 미분(Gradient) 정보만을 사용하여 최적점을 찾는 알고리즘으로, 순수 경사 하강법(Gradient Descent)이 가진
 * 지그재그(Zig-zag) 현상과 느린 수렴 속도를 개선하기 위해 고안되었습니다.
 * 
 * 가장 큰 특징은 이전의 탐색 방향과 새로운 탐색 방향이 서로 '공액(Conjugate, 직교를 일반화한 개념)'을 이루도록 
 * 방향을 설정한다는 것입니다. 이를 통해 N차원 2차 함수(Quadratic Function)의 경우 최대 N번의 반복 내에
 * 정확한 최적해를 찾을 수 있다는 수학적 보장이 있습니다.
 * 
 * 본 구현에서는 비선형 함수에 널리 사용되는 Fletcher-Reeves 방식을 채택하였으며, 
 * 학습률(Learning Rate, alpha)을 고정하지 않고 Exact Line Search(선 탐색)를 통해 매 스텝마다 최적의 보폭을 스스로 찾습니다.
 */
   ...
   public:
    ...
    static std::array<double, N> optimize(Func f, std::array<double, N> x, size_t max_iter = 1000,
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            // 기울기 크기(루트 g_norm_sq)가 지정된 허용 오차(tol)보다 작으면 극솟값에 도달했다고 판단합니다.
            ...
            // 사용자가 고정된 학습률(Learning Rate)을 지정하지 않고, 해당 방향(d)으로 가장 많이 감소할 수 있는 
            // 최적의 전진 거리(alpha)를 수학적으로 찾아냅니다.
            
            // 2-1. 브래킷 부호 변경(Bracket Sign Change)을 통해 방향 미분값이 0을 가로지르는(감소하다가 증가하는) 확실한 구간을 잡습니다.
            auto bracket = LineSearch::bracket_sign_change<N>(f, x, d, 0.0, 0.001, 2.0, false);
            
            // 2-2. 확보된 구간(bracket.first ~ bracket.second) 내에서 이분법(Bisection)을 수행하여 정확한 보폭 alpha를 산출합니다.
            double alpha =
            ...
            // 새로운 방향은 현재의 가장 가파른 하강 방향(-g_new)에 이전 방향(d)을 beta 비율만큼 섞어 만들어집니다.
            // 이를 통해 이전까지 탐색해온 계곡의 축(Axis) 정보를 잃지 않고 나아갈 수 있습니다.
            for (size_t i = 0; i < N; ++i) {
