...
namespace Optimization {
    class ConjugateGradient {
        public:
            ...
            static std::array<double, N> optimize(Func f, std::array<double, N> x,
                                                  size_t max_iter=1000, double tol=1e-4,
                                                  bool verbose=false) {
                if (verbose) {
                    std::cout << "========================================================\n";
                ...
                for (size_t iter = 1; iter <= max_iter; ++iter) {
                    ...
                    if (verbose && iter % 10 == 0) {
                        std::cout << "[Iter " << std::setw(3) << iter << "] f(x): " << std::fixed << std::setprecision(6) << f_x << "\n";
                    }
                }
                if (verbose) {
                    std::cout << "========================================================\n";
