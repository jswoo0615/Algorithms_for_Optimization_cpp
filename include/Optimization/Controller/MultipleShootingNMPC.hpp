...
namespace controller {
    ...
    struct NMPCResiduals {
        ...
        StaticVector<T, M> operator()(const StaticVector<T, Nz>& Z) const {
            ...
            auto get_x = [&](size_t k) {
                StaticVector<T, Nx> x;
                for(size_t i = 0; i < Nx; ++i) x(static_cast<int>(i)) = Z(static_cast<int>(k * (Nx + Nu) + i));
                return x;
            };
            auto get_u = [&](size_t k) {
                StaticVector<T, Nu> u;
                for(size_t i = 0; i < Nu; ++i) u(static_cast<int>(i)) = Z(static_cast<int>(k * (Nx + Nu) + Nx + i));
                return u;
            };
            ...
            for(size_t i = 0; i < Nx; ++i) {
                residuals(static_cast<int>(res_idx++)) = T(W_init) * (x_0(static_cast<int>(i)) - T(x_current(static_cast<int>(i))));
            }
            ...
            for(size_t k = 0; k < H; ++k) {
                ...
                for(size_t i = 0; i < Nx; ++i) {
                    ...
                    residuals(static_cast<int>(res_idx++)) = T(W_dyn) * (x_next(static_cast<int>(i)) - x_pred(static_cast<int>(i)));
                }
            }
            ...
            for(size_t k = 0; k <= H; ++k) {
                StaticVector<T, Nx> x_k = get_x(k);
                for(size_t i = 0; i < Nx; ++i) {
                    residuals(static_cast<int>(res_idx++)) = T(W_state) * (x_k(static_cast<int>(i)) - T(x_reference(static_cast<int>(i))));
                }
            }
