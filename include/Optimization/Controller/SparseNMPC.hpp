...
class SparseNMPC {
   public:
    ...
    SparseNMPC() {
        ...
        R_rate.set_zero();
        R_rate(0) = 50.0;
        R_rate(1) = 250.0;  // Slew rate penalty
        u_last.set_zero();
        dt = 0.1;
    ...
    StaticVector<T, 26> eval_node_residuals(const StaticVector<T, Nx>& x,
        ...
        res(idx++) =
            (Optimization::get_value(a_viol_upper) > 0.0) ? T(100.0) * a_viol_upper : T(0.0);
        res(idx++) =
            (Optimization::get_value(a_viol_lower) > 0.0) ? T(100.0) * a_viol_lower : T(0.0);

        // 조향 한계 [-0.5, 0.5] rad
        T steer_viol_upper = u(1) - T(0.5);
        T steer_viol_lower = T(-0.5) - u(1);
        res(idx++) = (Optimization::get_value(steer_viol_upper) > 0.0) ? T(100.0) * steer_viol_upper
                                                                       : T(0.0);
        res(idx++) = (Optimization::get_value(steer_viol_lower) > 0.0) ? T(100.0) * steer_viol_lower
                                                                       : T(0.0);
    ...
    bool solve(const StaticVector<double, Nx>& x_curr, const StaticVector<double, Nx>& x_ref) {
        ...
        for (size_t k = 0; k < H; ++k) {
            ...
            StaticVector<ADVar, 26> res_dual =
