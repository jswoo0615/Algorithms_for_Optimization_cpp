...
class SparseNMPC {
   public:
    ...
    SparseNMPC() {
        ...
        Q(3) = 10.0;
        Qf.set_zero();
        for (size_t i = 0; i < Nx; ++i) Qf(i) = Q(i) * 5.0;
        R.set_zero();
        R(0) = 1.0;
        R(1) = 20.0;
        R_rate.set_zero();
    ...
    NMPCResult solve(const StaticVector<double, Nx>& x_curr_global,
                     const StaticVector<double, Nx>& x_ref_global, int max_sqp_iter = 1) {
        NMPCResult result;
        ...
        for (int sqp = 0; sqp < max_sqp_iter; ++sqp) {
            ...
            for (size_t k = 0; k < H; ++k) {
                ...
                StaticVector<ADVar, 25> res_dual = eval_node_residuals(
                    x_dual, u_dual, u_prev_dual, x_ref_local, x_curr_global, x_ref_global);
            ...
            if (std::isnan(max_step) || max_step > 5.0)
