    ...
    public:
                               ...
                               const NMPCTuningConfig& config) 
        {
            ...
            for (size_t k = 0; k < H; ++k) {
                ...
                StaticVector<ADVar, 25> res_dual = eval_node_residuals(x_dual, u_dual, u_prev_dual, x_ref_local, x_curr_global, x_ref_global, config, k);
            ...
            for (size_t k = 0; k < H; ++k) {
                for (size_t i = 0; i < Nu; ++i) {
                    ...
                    if (du > 1e-4) {
                        ...
                        if (max_alpha > 0.0) alpha = std::min(alpha, config.safety_fraction * max_alpha);
                    } else if (du < -1e-4) {
            ...
            if (std::isnan(max_kkt) || max_kkt > 20.0) return execute_fallback(result, "Divergence Detected");
