    ...
    class CrossEntropy {
        ...
        public:
            ...
            [[nodiscard]] static OptimizationResultND<N> optimize(
                Func f, std::array<double, N> mu_init, std::array<double, N> sigma_sq_init, size_t max_iter = 100, double tol = 1e-5, uint32_t seed = 12345, bool verbose = false) noexcept {
                ...
                for (iter = 1; iter <= max_iter; ++iter) {
                    ...
                    if (verbose && (iter % 10 == 0 || iter == 1)) {
                        std::cout << "[Iter " << std::setw(3) << iter 
                                << "] Best f(x): " << std::fixed << std::setprecision(6) << population[0].y 
                                << " | Max Var: " << max_variance << "\n";
                    }

                    // 4. 종료 조건: 분산이 붕괴(Collapse)하여 한 점으로 수렴했는가?
                    if (max_variance < tol) {
                        if (verbose) std::cout << "  ↳ ✅ Converged (Variance collapsed) at Iteration: " << iter << "\n";
                        break;
                    }
                }
