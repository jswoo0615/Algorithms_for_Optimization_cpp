 ...
 * 립시츠 상수(Lipschitz constant)를 미리 알 필요 없이, 탐색 공간을 여러 개의 하이퍼큐브(초직육면체)로 분할하며
 * 전역 최솟값을 찾습니다.
 * 
 * 탐색 공간은 항상 정규화된 [0, 1]^N 공간에서 다루어지며, 목적 함수를 평가할 때만 원래의 범위 [a, b]로 변환(denormalize)합니다.
 * 
 * @note 본 구현은 동적 메모리 할당(`std::vector` 등)을 배제하고 `std::array`를 활용한 정적 메모리 풀(Static Pool) 패턴을 
 * 적용하여 실시간(Real-Time) 시스템 및 임베디드 환경에서의 안전성과 MISRA C++ 준수를 고려하였습니다.
 * 
 * @tparam N 최적화 문제의 차원 수
 * @tparam MAX_INTERVALS 분할 가능한 최대 하이퍼큐브(구간)의 개수 (기본값: 10000, 메모리 가용량에 따라 조절 필요)
 */
...
class DIRECT {
   private:
     ...
     */
    struct Interval {
        alignas(64) std::array<double, N> c;  ///< 하이퍼큐브의 중앙점 좌표 (정규화된 [0, 1]^N 공간 내의 위치)
         ...
         * @details DIRECT 알고리즘은 하이퍼큐브를 분할할 때, 가장 적게 분할된 차원(가장 긴 변)을 우선적으로 분할합니다.
...
#pragma omp simd
   ...
   public:
     ...
     * @param epsilon 잠재적 최적 구간(Potentially Optimal Interval)을 판단하기 위한 허용 오차 (기본값: 1e-4)
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> a,
        ...
        // 동적 메모리 할당(new, malloc)으로 인한 파편화나 지연을 막기 위해 thread_local로 큰 배열을 스택/데이터 영역에 할당
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // 간략화된 구현: 현재 최고 기록(y_best)에 가까운(epsilon 내에 있는) 구간들을 후보로 선정합니다.
            // (정석적인 DIRECT는 립시츠 상수 하한을 이용해 볼록 껍질(Convex Hull)을 구하여 식별합니다)
            ...
            for (size_t s_idx = 0; s_idx < S_size; ++s_idx) {
                ...
                // 한 번의 분할로 차원 당 2개(좌/우)의 새 구간이 생기므로, 메모리 풀 용량을 사전 검사
                if (pool_size + 2 * N > MAX_INTERVALS) {
                    break;
                }

                Interval& parent = pool[idx];
                size_t d = parent.min_depth(); // 분할을 진행할 깊이 (가장 적게 분할된 차원부터 분할)

                // 각 차원을 순회하며 해당 차원이 최소 분할 상태(d)라면 분할을 수행
                for (size_t dim = 0; dim < N; ++dim) {
                    if (parent.depths[dim] == d) {
                        // 3-등분할 때 중앙점의 이동 거리 계산
                        // 깊이 d일 때 한 변의 길이는 3^(-d)에 비례하므로 중앙점 간의 간격은 3^(-(d+1))이 됨
