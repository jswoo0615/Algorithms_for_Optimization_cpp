...
struct Dual {
    ...
    constexpr Dual operator+(const Dual& rhs) const { return {v + rhs.v, d + rhs.d}; }
    constexpr Dual operator-(const Dual& rhs) const { return {v - rhs.v, d - rhs.d}; }
    constexpr Dual operator*(const Dual& rhs) const { return {v * rhs.v, d * rhs.v + v * rhs.d}; }
    ...
    // 우측 스칼라 (Dual * Scalar)
    constexpr Dual operator+(const T& rhs) const { return {v + rhs, d}; }
    constexpr Dual operator-(const T& rhs) const { return {v - rhs, d}; }
    constexpr Dual operator*(const T& rhs) const { return {v * rhs, d * rhs}; }
    constexpr Dual operator/(const T& rhs) const { return {v / rhs, d / rhs}; }
};
...
struct DualVec {
    ...
    constexpr DualVec(const T& value = T(0)) : v(value) { g.fill(T(0)); }
    ...
    constexpr DualVec operator+(const T& rhs) const { return *this + DualVec(rhs); }
    constexpr DualVec operator-(const T& rhs) const { return *this - DualVec(rhs); }
