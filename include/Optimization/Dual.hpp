...
/**
 * @brief 전진 모드 자동 미분(Forward-mode Automatic Differentiation)을 위한 스칼라 듀얼(Dual) 수 구조체
 * @details 
...
struct Dual {
    ...
    // --- 제자리(In-place) 연산자 ---
    // 객체를 새로 생성(메모리 재할당)하지 않고 현재 객체에 바로 연산 결과를 반영하여 성능을 최적화합니다.
 ...
 * @details 
 * 다변수 함수 f(x1, x2, ..., xn)에 대하여 함수 값과 N차원 기울기 벡터(Gradient)를 동시에 추적합니다.
 * 스택 할당(std::array)을 사용하여 힙 메모리 할당으로 인한 오버헤드가 없습니다.
 * 
 * @tparam T 데이터 타입
 * @tparam N 변수의 차원 (개수)
 */
 ...
 * @details 0 근처에서의 특이점(Singularity) 문제를 방지하기 위해 v <= 1e-16 일 경우 미분값을 0으로 처리합니다.
 */
template <typename T>
 ...
 * 이 매크로는 std::complex에 대해 Chain Rule을 적용하여 CSD가 초월 함수를 안전하게 통과하도록 확장합니다.
 */
