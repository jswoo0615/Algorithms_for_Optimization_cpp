...
/**
 * @brief 등식 제약 조건이 있는 이차 계획법(Equality-Constrained Quadratic Programming, EQP) 솔버 클래스
 * @details 
 ...
 * P는 양의 정부호(Positive Definite) 혹은 양의 준정부호(Positive Semi-Definite) 대칭 행렬인 헤시안(Hessian) 행렬입니다.
 * 
 * 이 문제는 라그랑주 승수법(Lagrange Multipliers)을 사용하여 KKT(Karush-Kuhn-Tucker) 시스템이라는 
 * 선형 연립방정식을 블록 행렬 형태로 구성하고, 그 해를 구함으로써 해석적으로 단번에(closed-form) 최적해를 얻을 수 있습니다.
 * 주로 비선형 제어(NMPC)나 순차적 이차 계획법(SQP), 내점법(Interior Point Method)의 내부 부분 문제(Subproblem)로 활용됩니다.
 * 
...
class EQPSolver {
   public:
     ...
     * @details 제약 조건 방정식 A u = b 에서 좌변 행렬을 구성합니다. (예: 동역학 모델의 선형화 된 상태 전이 행렬 등)
     */
   ...
   private:
    ...
    // 동적 힙 메모리 할당(new/malloc)을 배제하여 실시간(Real-Time) 제어 환경에서의 안전성과 속도를 확보합니다.
    ...
    StaticVector<double, KKT_Size> KKT_solution;         ///< KKT 시스템의 전체 해 벡터 (u 와 lambda 가 병합되어 있음)
   ...
   public:
     ...
     * KKT 행렬은 우하단 블록이 영행렬(0)이므로 전체적으로 양의 정부호(Positive Definite)가 될 수 없는
     * 대칭 부정부호 행렬(Symmetric Indefinite Matrix, 안장점 구조)입니다.
     * 따라서 일반적인 Cholesky (LL^T) 분해는 불가능하며 LDL^T 분해를 사용해야 합니다.
     * 제약 조건 행렬 A의 행(Row)들이 선형 독립(Linear Independent)이 아니거나, P가 제약조건의 
     * Null Space에서 충분히 양의 정부호가 아닐 경우 분해가 터지거나(실패) 특이 행렬(Singular)이 될 수 있습니다.
     * 
    ...
    bool solve() {
        ...
        // 이전 루프에서 덮어씌워진 찌꺼기(데이터 쓰레기값)가 남지 않도록 완전히 0으로 초기화해야 합니다.
        KKT_Matrix.set_zero();
        KKT_rhs.set_zero();
        ...
        // KKT 해 벡터의 앞부분(상단 N_vars 개)은 실제 우리가 구하고자 하는 최적 제어 변수(u) 입니다.
