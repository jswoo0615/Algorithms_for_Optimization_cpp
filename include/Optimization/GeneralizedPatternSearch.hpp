 ...
 * @details
 * 기울기(Gradient) 정보를 사용하지 않는(Derivative-free) 직접 탐색(Direct Search) 방법 중
 * 하나입니다. 주어진 위치 주변을 특정 방향 집합(Positive Spanning Set)을 따라 탐색하며, 더 나은
 * 지점(목적 함수값이 작은 지점)을 찾으면 즉시 해당 지점으로 이동(기회주의적 업데이트)합니다. 만약
 * 모든 방향을 다 뒤져도 더 나은 지점을 찾지 못하면 보폭(Step size, alpha)을 줄여 더 세밀하게
 * 탐색합니다.
 *
 * @note 본 구현은 두 가지 최적화 기법을 도입하여 실시간 제어 환경에서의 속도와 강건성을
 * 확보했습니다.
 ...
 *    일반적으로 볼록한 함수에서 한 번 내려가기 시작한 방향은 다음 스텝에서도 또 내려갈 확률이 높기
 * 때문입니다. 이로 인해 평균적인 목적 함수 평가 횟수를 크게 줄일 수 있습니다.
 * 3. MISRA C++ 준수 및 SIMD/FMA 가속:
 *    정적 배열(std::array)을 사용해 동적 메모리 할당(힙)을 원천 차단하고 메모리 단편화를
 * 방지합니다.
 */
class GeneralizedPatternSearch {
   public:
     ...
     *          이 벡터들은 공간의 모든 방향을 양의 계수로 조합해낼 수 있어야 합니다. (예: 축 방향
     * 및 그 반대 방향)
     * @param alpha 초기 탐색 보폭 (Step size). (기본값: 1.0)
     * @param epsilon 수렴 판정 기준치. 보폭(alpha)이 이 값보다 작아지면 최적점에 도달했다고
     * 판단하고 종료합니다. (기본값: 1e-5)
     * @param gamma 보폭 축소 비율. 주변 탐색에 실패했을 때 보폭을 얼마나 줄일 것인지 결정합니다.
     * (기본값: 0.5)
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> x_init,
        ...
        while (alpha > epsilon && iter < max_iter) {
            ...
            for (size_t i = 0; i < M; ++i) {
                ...
                // SIMD 지시어와 FMA(Fused Multiply-Add) 연산을 사용하여 부동소수점 오차를 줄이고
                // 병렬로 빠르게 연산
#pragma omp simd
                ...
                if (y_prime < y) {
                    ...
                    // 이 방향 벡터를 탐색 우선순위 1위(배열의 맨 앞, index 0)로
                    // 재배치(Shift)합니다. 이는 불필요한 함수 평가를 획기적으로 줄여주는 휴리스틱
                    // 기법입니다.
                    if (i > 0) {
                    ...
                    // 기회주의적 탐색이므로 한 번 성공하면 현재 루프를 빠져나가고 다음 step으로
                    // 넘어감
                    break;
                }
            }
            ...
            // 현재 지점이 골짜기의 바닥 근처이거나 국소 최적점(Local Minimum)에 가까워졌다는
            // 뜻입니다. 이때는 보폭(alpha)에 축소 비율(gamma, 주로 0.5)을 곱해 탐색 반경을 세밀하게
            // 좁힙니다.
            if (!improved) {
                alpha *= gamma;
            }
