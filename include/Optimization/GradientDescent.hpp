...
class GradientDescent {
   public:
     ...
     * @param tol 수렴 판정을 위한 허용 오차 (Gradient의 L2 Norm이 이 값보다 작아지면 종료). 기본값:
     * 1e-6
     * @param max_iter 최대 반복 횟수. 이 횟수에 도달하면 알고리즘이 강제 종료됩니다. 기본값: 1000
     * @param verbose 진행 과정 출력 여부. true일 경우 매 반복마다 상태를 콘솔에 출력합니다. 기본값:
     * false
     *
    ...
    static std::array<double, N> optimize(Func f, std::array<double, N> x, double tol = 1e-6,
        ...
        for (int i = 0; i < max_iter; ++i) {
            ...
            // AutoDiff::value_and_gradient를 사용하여 자동 미분 기반으로 정확한 함수값과 기울기를
            // 얻습니다.
            double f_val;
            ...
            // 기울기의 벡터 크기(|grad|)를 통해 현재 위치가 충분히 평탄한지(극솟값에 도달했는지)
            // 확인합니다.
            double g_norm = 0.0;
            for (double g : grad) {
                g_norm += g * g;
            ...
            // 고정된 보폭(Learning Rate) 대신 Strong Wolfe 조건을 만족하는 Backtracking Line
            // Search를 사용하여 충분한 함수값 감소를 보장하는 최적의 이동 거리(alpha)를 탐색합니다.
