 ...
 * 
 * @note 미분 (Gradient) 없이 탐색 (Exploratory)과 패턴 이동 (Pattern)을 결합하여 탐색 속도를 가속합니다.
 * 성능 향상을 위해 힙 동적 할당을 차단하고, 캐시 라인 정렬 (alignas(64))을 적용하여 최적화되었습니다.
 */
class HookeJeeves {
   private:
    /**
     * @brief 탐색 이동 (Exploratory Move / 더듬기)
     * @details 현재 위치에서 각 차원(축)을 기준으로 양의 방향과 음의 방향으로 순차적으로 탐색합니다.
     * 한 축 방향으로의 이동이 함수값을 감소시킨다면(개선된다면), 즉시 그 위치를 확정하고 다음 축 탐색을 이어갑니다.
     * 
     * @tparam N 차원 수
   ...
   public:
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> x_start,
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // 부동 소수점 좌표 배열 자체를 비교하는 대신, 목적 함수값의 개선 여부로 탐색 성공을 판별합니다.
            // 이는 부동소수점 오차에 대해 더 강건(Robust)합니다.
            if (f_best < f_prev) {
...
#pragma omp simd
                ...
                if (f_pattern < f_best) {
                    // 패턴 이동 성공: 더 좋은 결과를 얻었으므로 베이스 위치와 최적값을 도약 위치로 갱신
                    x = x_pattern_new;
                    f_best = f_pattern;
                } else {
                    // 패턴 이동 실패: 도약이 오히려 안 좋은 결과를 낳았으므로 단순 탐색 결과(x_new)만 적용 (롤백)
                    x = x_new;
                }
            } else {
                ...
                // 보폭이 설정된 허용 오차(tol)보다 작아지면 충분히 극솟값에 도달했다고 판단하고 종료합니다.
                if (alpha < tol) {
