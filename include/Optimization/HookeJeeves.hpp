...
namespace Optimization {
    ...
    class HookeJeeves {
        private:
            ...
            static constexpr std::array<double, N> explore(
                Func f, std::array<double, N> x, double alpha, double& f_best) noexcept {
        ...
        public:
            ...
            [[nodiscard]] static OptimizationResultND<N> optimize(
                ...
                double tol = 1e-6, size_t max_iter = 2000, bool verbose = false) noexcept {
                ...
                for (iter = 1; iter <= max_iter; ++iter) {
                    ...
                    if (f_best < f_prev) {
                        ...
                        alignas(64) std::array<double, N> x_pattern_new = explore<N>(f, x_pattern, alpha, f_pattern);
                    ...
                    if (verbose && (iter % 100 == 0 || iter == 1)) {
                        std::cout << "[Iter " << std::setw(4) << iter 
                                << "] f(x): " << std::fixed << std::setprecision(6) << f_best 
                                << " | Step(alpha): " << alpha << "\n";
                    }
                }
