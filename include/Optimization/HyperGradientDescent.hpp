  ...
  class HyperGradientDescent {
    public:
      ...
      [[nodiscard]] static constexpr std::array<double, N> optimize(
          Func f, std::array<double, N> x,
          double alpha_init = 0.001, double mu = 1e-6, double tol = 1e-5,
          size_t max_iter = 50000, bool verbose = false) noexcept {
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
          ...
          if (std::isnan(f_x) || std::isinf(f_x)) {
              if (verbose) std::cout << " ⚠️ Warning: Numerical explosion detected. Rolling back.\n";
              return x_valid;
          }
