...
class HyperGradientDescent {
   public:
    ...
    [[nodiscard]] static constexpr std::array<double, N> optimize(
        ...
        constexpr double MAX_ALPHA =
            0.005;  // Rosenbrock과 같은 급경사 함수에서 발산하는 것을 막기 위한 보수적 상한치
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            // 목적 함수의 현재 위치(x)에서의 값(f_x)과 기울기(g)를 계산합니다. (메모리 재할당 없음
            // O(1))
...
#pragma omp simd
            for (size_t i = 0; i < N; ++i) {
                ...
                g_dot_gprev =
                    std::fma(g[i], g_prev[i], g_dot_gprev);  // g_dot_gprev += g[i] * g_prev[i]
            }

            // 3. 수렴(종료) 조건 검사
            // 기울기의 크기 제곱이 목표 허용 오차 제곱보다 작다면 (즉, 극소점에 충분히
            // 가까워졌다면) 종료
            if (g_norm_sq < tol_sq) {
                break;
            }
            ...
            // g_dot_gprev (이전 기울기와 현재 기울기의 내적)이 양수이면, 같은 방향으로 가고
            // 있으므로 보폭(alpha) 증가. 음수이면, 계곡을 지그재그로 넘나들고(Overshooting) 있다는
            // 뜻이므로 보폭(alpha) 감소. 이때 클리핑된 스케일을 제곱하여 보폭 업데이트에도 안전성을
            // 반영합니다.
