...
class IPMQPSolver {
   public:
    ...
    bool solve(StaticVector<double, N_vars>& x, int max_iter = 50, double tol = 1e-4) {
        StaticVector<double, (N_eq > 0 ? N_eq : 1)> y;
        y.set_zero();
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            if constexpr (N_ineq > 0) {
                for (size_t i = 0; i < N_ineq; ++i) {
                    ...
                    K(static_cast<int>(i + N_vars + N_eq), static_cast<int>(i + N_vars + N_eq)) =
                        -s(static_cast<int>(i)) / z(static_cast<int>(i));
                }
            }
            ...
            if constexpr (N_ineq > 0) {
                for (size_t i = 0; i < N_ineq; ++i) {
                    ...
                    rhs(static_cast<int>(i + N_vars + N_eq)) =
                        -r_ineq(static_cast<int>(i)) + r_c / z(static_cast<int>(i));
                }
            }
            ...
            for (size_t i = 0; i < N_vars; ++i)
                dx(static_cast<int>(i)) = delta(static_cast<int>(i));
            if constexpr (N_eq > 0)
                for (size_t i = 0; i < N_eq; ++i)
