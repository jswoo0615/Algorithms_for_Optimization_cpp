...
namespace Optimization {
    class LineSearch {
        ...
        public:
            ...
            // ================================================================================
            template <size_t N, typename Func>
            static double golden_section_search(Func f, const std::array<double, N>& x, const std::array<double, N>& d, double a, double b, double tol=1e-5, bool verbose=false) {
                ...
                while (std::abs(d_step) > tol) {
                    iter++;
                    if (verbose) {
                        std::cout << "  ↳ [Iter " << std::setw(2) << iter << "] Range: [" << std::fixed << std::setprecision(6) << a
                                  << ", " << b << "] | Gap: " << std::abs(b - a) << "\n";
                    }
            ...
            // 국소 최솟값 부근으로 충분히 확대해 들어가면 대부분의 부드러운 목적 함수는 2차 함수 (Quadratic function),
            // 즉 U자 형태의 그릇 모양의 띄게 됩니다.
            // 이 점에 착안아혀, 현재 파악한 세 개의 점을 지나는 2차 함수 곡선을 가상으로 그리고,
            // 그 가상의 2차 함수의 최솟값을 다음 탐색 지점으로 바로 점프하는 방법
            //
            // 수학적 원리
            // - 최솟값을 감싸고 있는 세 개의 브레킷 지점 a < b < c와 그 지점들에 대한 함수값 ya, yb, yc
            // - 이 세 점을 완벽하게 지나는 2차 함수 q(x) = p1 + p2 * x + p3 * x^2 형태의 행렬 역연산 등을 통해 찾아냅니다
            // - 2차 함수의 최솟값은 미분값이 0이 되는 지점이므로, 수식을 풀면 해석적으로 유일한 최솟값 x^{*}의 위치를 계산 가능
            // ======================================
            template <size_t N, typename Func>
            static double quadratic_fit_search(Func f, const std::array<double, N>& x, const std::array<double, N>& d, double a, double b, double c, size_t max_iter=50, double tol=1e-5, bool verbose=false) {
                ...
                for (size_t iter = 1; iter <= max_iter; ++iter) {
                    ...
                    if (verbose) {
                        std::cout << "  ↳ [Iter " << std::setw(2) << iter << "] x*: " << std::fixed << std::setprecision(6) << x_star
                                  << " | f(x*): " << y_star << " | Shift: " << std::abs(x_star - b) << "\n";
                    }
            ...
            // 특이점 : 구간 내 골짜기가 몇 개가 있든 상관없이 반드시 진짜 바닥인 '전역 최솟값'을 확정적으로 찾아내는 전역 최적화 알고리즘
            // 1. 전제 조건 : 립시츠 상수 - 함수의 기울기 (가파른 정도)가 아무리 커져도 절대 넘을 수 없는 최대 한계치인 lIPSCHITZ CONSTANT를 미리 알고 있어야 함
            // 2. 핵심 원리 : 톱니바퀴 모양의 하한선 만들기
            // ==================================================================
            template <size_t N, size_t MAX_NODES=100, typename Func>
            static double shubert_piyavskii(Func f, const std::array<double, N>& x, const std::array<double, N>& d, double a, double b, double L, double tol=1e-4, bool verbose=false) {
                auto eval = [&](double alpha) {
                    return AutoDiff::value<N>(f, ray_point<N>(x, d, alpha));
                };
                struct SPNode {
                ...
                for (iter = 1; iter <= MAX_NODES / 2; ++iter) {
                    ...
                    if (verbose) {
                        std::cout << "  ↳ [Iter " << std::setw(2) << iter << "] Probe x_m: " << std::fixed << std::setprecision(5) << x_m
                                  << " | LowerBound y_m: " << pool[best_idx].y_m << " | Gap: " << std::abs(y_eval - pool[best_idx].y_m) << "\n";
                    }
            ...
            static std::pair<double, double> bracket_sign_change(Func f, const std::array<double, N>& x, const std::array<double, N>& d, double a, double b, double k=2.0, bool verbose=false) {
                ...
                while (fp_a * fp_b > 0.0) {
                    ...
                    if (verbose) {
                        std::cout << "  ↳ [Iter " << std::setw(2) << iter << "] Expanding: [" << std::fixed << std::setprecision(5) << a
                                  << ", " << b << "] | f'(a)=" << std::showpos << fp_a << " | f'(b)=" << fp_b << std::noshowpos << "\n";
                    }
                }
            ...
            static double bisection(Func f, const std::array<double, N>& x, const std::array<double, N>& d, double a, double b, double tol=1e-5, bool verbose = false) {
                ...
                while ((b - a) > tol) {
                    ...
                    if (verbose) {
                        std::cout << "  ↳ [Iter " << std::setw(2) << iter << "] Root Search: [" << std::fixed << std::setprecision(6) << a
                                  << ", " << b << "] | Mid: " << mid << " | f'(mid): " << std::showpos << y_mid << std::noshowpos << "\n";
                    }
