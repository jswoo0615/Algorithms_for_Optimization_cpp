 ...
 * 
 * @details MADS 알고리즘은 함수의 미분 정보(기울기 등)를 사용하지 않는 Derivative-Free Optimization(DFO) 기법입니다.
 * 목적 함수의 평가(Evaluation)만을 기반으로 탐색을 진행하므로, 미분 불가능한 지점이 있거나 노이즈가 많은 환경에서도 강건하게 동작합니다.
 * 본 구현체는 매 스텝마다 탐색 방향으로 무작위 'Positive Spanning Set(양의 생성 집합)'을 생성하여 지역 최솟값에 빠지는 것을 방지하며 탐색 범위를 조절합니다.
 * 
 * @note 탐색 기저 생성에 무작위성(Randomness)이 포함되어 있으므로 결과가 비결정론적일 수 있습니다.
 *       따라서 엄격한 시간 제약이 있는 실시간(Real-Time) 제어 루프 내부보다는 오프라인 궤적 생성 및 파라미터 튜닝용으로 적합합니다.
 */
class MeshAdaptiveDirectSearch {
   private:
    /**
     ...
     * 마지막 N+1번째 벡터는 나머지 모든 기저 벡터들의 역방향 합으로 설정하여 공간 전체를 커버할 수 있도록 만듭니다.
     * 정적 배열(std::array)을 사용하여 O(1) 복잡도로 메모리 지연 없이 빠르게 동작합니다.
     * 
    ...
    [[nodiscard]] static std::array<std::array<double, N>, N + 1> generate_positive_spanning_set(
        ...
        for (size_t d_idx = 0; d_idx < N; ++d_idx) {
            for (size_t dim = 0; dim < N; ++dim) {
                ...
                // 이렇게 해야 N+1개의 벡터가 공간의 모든 방향을 양의 조합(Positive Span)으로 표현할 수 있음
                D[N][dim] -= val;  
            }
        }
        return D;
   ...
   public:
     ...
     * @return 최적해(x_opt), 최적값(f_opt), 반복 횟수, 연산 시간을 포함하는 OptimizationResultND 구조체
     */
...
#pragma omp simd
                for (size_t j = 0; j < N; ++j) {
                    // x_prime = x + alpha * D[i] 
                    // std::fma(Fused Multiply-Add)를 사용하여 하드웨어 레벨에서 단일 명령어로 고속/고정밀 계산
...
#pragma omp simd
                    ...
                    // 더 나은 점을 하나라도 발견했으므로, 나머지 방향 평가는 건너뛰고 바로 루프 탈출
                    // (기회주의적 탐색 방식이 탐색 횟수를 획기적으로 줄여줌)
                    break;  
                }
            }
