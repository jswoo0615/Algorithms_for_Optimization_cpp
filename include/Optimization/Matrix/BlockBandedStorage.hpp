...
/**
 * @brief NMPC (Nonlinear Model Predictive Control) 최적화 문제에서 단일 예측 스텝 (t = k)의
 * 데이터를 담는 블록 구조체.
 *
 * LQR/Riccati 방정식 기반의 솔버에서 각 시점(stage)마다 필요한 선형화된 시스템 동역학,
 * 2차 비용 함수의 가중치, 그리고 최적화 수행 후 얻어지는 제어 이득(gain) 행렬들을 한 곳에 모아
 * 관리합니다. 이 구조체는 메모리를 연속적이고 정적으로 할당하기 위해 동적 할당 없이
 * 배열(StaticMatrix/StaticVector)만을 사용합니다.
 *
...
struct NMPCStorageData {
    ...
    /** @brief 상태 행렬 (State Transition Matrix). 이전 상태 x_k 가 다음 상태 x_{k+1} 에 미치는
     * 영향을 나타냄 */
    StaticMatrix<double, Nx, Nx> A;

    /** @brief 제어 입력 행렬 (Control Input Matrix). 현재 제어 입력 u_k 가 다음 상태 x_{k+1} 에
     * 미치는 영향을 나타냄 */
    StaticMatrix<double, Nx, Nu> B;

    /** @brief 비선형 시스템의 선형화 과정에서 발생하는 영점 잔차 또는 외란 (Linearization
     * residual/Affine term) */
    ...
    /** @brief 상태 변수에 대한 2차 가중치 행렬 (State Cost Weight Matrix). 주로 양의
     * 준정부호(Positive Semi-Definite) 사용 */
    StaticMatrix<double, Nx, Nx> Q;

    /** @brief 제어 입력에 대한 2차 가중치 행렬 (Control Cost Weight Matrix). 주로 양의
     * 정부호(Positive Definite) 사용 */
    StaticMatrix<double, Nu, Nu> R;

    /** @brief 상태 변수에 대한 1차 선형 비용 벡터 (Linear State Cost). 목표 궤적 추종 시 추종
     * 오차를 나타내는데 사용됨 */
    ...
    /** @brief 피드백 제어 이득 행렬 (Feedback Gain Matrix). 상태 오차를 보정하여 최적 제어를
     * 수행하게 함 */
    StaticMatrix<double, Nu, Nx> K;

    /** @brief 피드포워드 제어 이득 벡터 (Feedforward Gain Vector). 예측된 오차를 보상하기 위한 기본
     * 제어 입력 성분 */
