...
namespace Optimization {
    ...
    struct MathTraits {
         ...
         * 
         * [수치적 안정성 판단]
         * 선형 대수 분해 (LU, Cholesky, LDLT, QR 등) 과정에서 (Pivot, 대각 원소)으로 나누는 연산이 빈번합니다.
         * 이때 피벗이 수학적 0은 아니더라도 0에 극도로 가까운 수학적 노이즈라면 (특이 행렬, Singular),
         * 나눗셈 결과가 무한대로 발산하여 시스템이 붕괴합니다. 
         * 따라서 `std::numeric_limits<T>::epsilon()`을 사용하여 하드웨어 부동소숫점 아키텍처에 맞는
    ...
    struct MathTraits<Optimization::Dual<T>> {
         ...
         * 행렬 원소의 `미분값 (gradient/hessian)`이 아니라 오직 순수 `함수값 (Value, 실수부)`에 의해서만 결정되어야 합니다
    ...
    StaticMatrix<T, Rows, Cols> operator+(const StaticMatrix<T, Rows, Cols>& A, const StaticMatrix<T, Rows, Cols>& B) {
    ...
    StaticMatrix<T, Rows, Cols> operator/(const StaticMatrix<T, Rows, Cols>& A, T scalar) {
         ...
         * 압도적으로 느립니다. 루프 내에서 N*M 번 나누는 대신, 먼저 스칼라의 역수 (Inverse)를 단 1번만 구하고,
    ...
    StaticMatrix<T, Rows, OtherCols> operator*(const StaticMatrix<T, Rows, Cols>& A, const StaticMatrix<T, Cols, OtherCols>& B) {
        ...
        for (size_t j = 0; j < OtherCols; ++j) {
            ...
            for (size_t k = 0; k < Cols; ++k) {
                ...
                const b_kj = B(static_cast<int>(k), static_cast<int>(j));   // I 루프 동안 곱해질 B의 스칼라 값 고정
    ...
    namespace linalg {
         ...
         * 수학적으로 x = A^{-1} * b 이지만, 컴퓨터로 직접 A의 역행렬 (Inverse)을 통ㅎ째로 구하는 것은 
         * O(N^3)의 막대한 연산량이 소모되며 무엇보다 부동소숫점 오차가 기하급수적으로 누적되어 시스템이 불안정해집니다 (수치적 불안정성)
         * 따라서 현대 수치해석에서는 반드시 행렬 A를 다루기 쉬운 두세 개의 삼각 행렬 조각으로 "분해 (Decomposition)"한 뒤, 
         * 전진 대입 (Forward Substitution)과 후진 대입 (Backward Substitution)이라는 아주 싼 연산 (O(N^2))을 통해
         * 빠르고 정확하게 해 (x)를 구합니다.
         */
        template <typename T, size_t Rows, size_t Cols>
        bool LU_decompose(StaticMatrix<T, Rows, Cols>& mat) {
             ...
             * 
             * 모든 정방 행렬 (Square Matrix) A를 단위 하삼각행렬 L (Lower)과 상삼각행렬 U(Upper)의 곱 (A = L * U)으로 나눕니다
             * Doolittle 알고리즘을 사용하며, 메모리를 아끼기 위해 (제한된 임베디드 자원 고려) 추가적인 L, U 행렬 공간을 만들지 않고,
             * 원본 행렬 A의 메모리 공간에 분해된 결괏값들을 그대로 덮어씁니다 (In-place 방식)
             * 
            ...
            for (size_t i = 0; i < Rows; ++i) {
                for (size_t j = 0; j < Cols; ++j) {
                    ...
                    for (size_t k = 0; k < i; ++k) {
                        sum += mat(static_cast<int>(i), static_cast<int>(k)) * mat(static_cast<int>(k), static_cast<int>(j));
                    }
                ...
                for (size_t j = i + 1; j < Rows; ++j) {
                    T sum = static_cast<T>(0);
                    for (size_t k = 0; k < i; ++k) {
                        sum += mat(static_cast<int>(j), static_cast<int>(k)) * mat(static_cast<int>(k), static_cast<int>(i));
                    }
                    // 미리 구해둔 피벗 U_ii로 나누어 스케일링
                    mat(static_cast<int>(j), static_cast<int>(i)) = (mat(static_cast<int>(j), static_cast<int>(i)) - sum) / mat(static_cast<int>(i), static_cast<int>(i));
                }
            }
            return true;
        }
         ...
         * 위에서부터 아래로 미지수를 순차적으로 찾기 쉬우므로 전진 대입 (Forward Substitution)이라 부릅니다.
         * 
         * 2단계 : U (상삼각행렬) * x = y를 풉니다. 
         * 밑에서부터 위로 거꾸로 올라가며 미지수를 찾아 해 x를 확정하므로 후진 대입 (Backward Substitution)이라 부릅니다
         * 
        ...
        template <typename T, size_t Rows, size_t Cols>
        StaticVector<T, Rows> LU_solve(const StaticMatrix<T, Rows, Cols>& mat, const StaticVector<T, Rows>& b) {
            ...
            for (size_t i = 0; i < Rows; ++i) {
                ...
                // Doolittle LU에서 L의 대각 원소는 암묵적으로 항상 1.0이므로, 나눗셈 처리가 아예 생략되어 연산이 매우 빠릅니다
                y(i) = b(i) - sum;
            }
         ...
         * 
         * [제약 조건 및 강점]
         * 대상 행렬 A가 반드시 대칭 행렬 (Symmetric, A = A^T)이고, 동시에 양의 정부호 (Positive Definite, 모든 고윳값이 양수)
         * 일 때만 수학적으로 성립하는 특수 분해 기법입니다. 
         * 일반 LU 분해에 비해 필요한 반복 연산량이 정확히 절반 (O(N^3/3))으로 줄어들며, 부동소숫점 오차에 대한 수치적 안정성 (Numerical Stability)이 압도적으로 좋습니다.
         * SQP 최적화나 Gauss-Newton (J^T J 구조) 형태의 역삭/제어 문제를 풀 때, Hessian 행렬은 기본적으로 양의 정부호 대칭성을 띠므로
         * 이때 가장 강력한 최우선 솔버로 활용됩니다.
        ...
        bool Cholesky_decompose(StaticMatrix<T, Rows, Cols>& mat) {
            ...
            for (size_t j = 0; j < Cols; ++j) {
                ...
                for (size_t k = 0; k < j; ++k) {
                    s += mat(static_cast<int>(j), static_cast<int>(k)) * mat(static_cast<int>(j), static_cast<int>(k));
                }
                ...
                for (size_t i = j + 1; i < Rows; ++i) {
                    ...
                    for (size_t k = 0; k < j; ++k) {
                        s_ij += mat(static_cast<int>(i), static_cast<int>(k)) * mat(static_cast<int>(j), static_cast<int>(k));
                    }
                    mat(static_cast<int>(i), static_cast<int>(j)) = (mat(static_cast<int>(i), static_cast<int>(j)) - s_ij) / mat(static_cast<int>(j), static_cast<int>(j));
                }
            }
            return true;
        }
         ...
         * 본 엔진은 제어 솔버 (NMPC, SQP) 내부망에 통합되어 있으므로 알고리즘의 붕괴를 사전에 막기 위해 
         * D의 원소가 엄격한 양수 (Positive Definite)인지 입실론 수준에서 강하게 검열합니다.
        ...
        bool LDLT_decompose(StaticMatrix<T, Rows, Cols>& mat) {
            ...
            for (size_t j = 0; j < Cols; ++j) {
                ...
                for (size_t i = j + 1; i < Rows; ++i) {
                    ...
                    for (size_t k = 0; k < j; ++k) {
                        sum_L += mat(static_cast<int>(i), static_cast<int>(k)) * mat(static_cast<int>(j), static_cast<int>(k)) * mat(static_cast<int>(k), static_cast<int>(k));
                    }
                    mat(static_cast<int>(i), static_cast<int>(j)) = (mat(static_cast<int>(i), static_cast<int>(j)) - sum_L) / mat(static_cast<int>(j), static_cast<int>(j));
                }
            }
            return true;
        }
        ...
        template <typename T, size_t Rows, size_t Cols>
        StaticVector<T, Rows> LDLT_solve(const StaticMatrix<T, Rows, Cols>& mat, const StaticVector<T, Rows>& b) {
            static_assert(Rows == Cols, "Solve requires a square matrix");
             ...
             * 이 한 줄의 나눗셈 과정이 Cholesky 분해의 무거운 무리수 루트 (sqrt) 연산을 대체하는 수학적 역할입니다.
             */
         ...
         * 기하급수적으로 쌓여 최종 도출된 Q 벡터들이 직교성을 상실하고 한쪽으로 무너지는 끔찍한 단점이 있습니다.
         * MGS 알고리즘은 이를 방지하기 위해, 하나의 직교 Q열을 찾아낼 때마다 그 직교성을 "아직 처리하지 않은 모든 나머지 열들에서"
        ...
        bool QR_decompose_MGS(StaticMatrix<T, Rows, Cols>& mat, StaticMatrix<T, Cols, Cols>& R) {
            ...
            for (size_t i = 0; i < Cols; ++i) {
                ...
                for (size_t k = 0; k < Rows; ++k) {
                    norm_sq += mat(static_cast<int>(k), static_cast<int>(i)) * mat(static_cast<int>(k), static_cast<int>(i));
                }
                ...
                for (size_t k = 0; k < Rows; ++k) {
                    mat(static_cast<int>(k), static_cast<int>(i)) /= R(static_cast<int>(i), static_cast<int>(i));
                }

                for (size_t j = i + 1; j < Cols; ++j) {
                    T dot = static_cast<T>(0);
                    for (size_t k = 0; k < Rows; ++k) {
                        dot += mat(static_cast<int>(k), static_cast<int>(i)) * mat(static_cast<int>(k), static_cast<int>(i));
                    }
                    R(static_cast<int>(i), static_cast<int>(j)) = dot;

                    for (size_t k = 0; k < Rows; ++k) {
                        mat(static_cast<int>(k), static_cast<int>(j)) -= dot * mat(static_cast<int>(k), static_cast<int>(i));
                    }
                }
            }
            return true;
        ...
        template <typename T, size_t Rows, size_t Cols>
        StaticVector<T, Cols> QR_solve(const StaticMatrix<T, Rows, Cols>& mat, const StaticMatrix<T, Cols, Cols>& R, const StaticVector<T, Rows>& b) {
