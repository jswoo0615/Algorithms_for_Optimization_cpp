...
inline MathStatus QR_decompose_MGS(StaticMatrix<T, Rows, Cols>& mat,
                                   StaticMatrix<T, Cols, Cols>& R) {
...
// --- SIMD 가속: Dot Product ---
        ...
        for (size_t j = i + 1; j < Cols; ++j) {
            ...
            k = 0;
            // SIMD 내적 (mat_col_i * mat_col_j) - 스칼라 폴백으로 축약 표현 (실제 구현 시 위와
            // 동일한 구조)
...
// =======================================================================================
template <typename T, size_t Rows, size_t Cols>
inline MathStatus QR_decompose_Householder(StaticMatrix<T, Rows, Cols>& mat,
                                           StaticVector<T, Cols>& tau) {
    ...
    for (size_t i = 0; i < Cols; ++i) {
        ...
        T m_ii = mat(i, i);
        T sign =
            (Optimization::get_value(m_ii) >= 0.0) ? static_cast<T>(1.0) : static_cast<T>(-1.0);
...
inline void QR_solve_Householder(const StaticMatrix<T, Rows, Cols>& mat,
                                 const StaticVector<T, Cols>& tau, const StaticVector<T, Rows>& b,
