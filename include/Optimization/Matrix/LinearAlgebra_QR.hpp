        ...
        MathStatus QR_decompose_Householder(StaticMatrix<T, Rows, Cols>& mat, StaticVector<T, Cols>& tau) {
            ...
            for (size_t i = 0; i < Cols; ++i) {
                ...
                for (size_t j = i + 1; j < Cols; ++j) {
                    ...
                    for (size_t k = i + 1; k < Rows; ++k) {
                        dot += mat(static_cast<int>(k), static_cast<int>(i)) * mat(static_cast<int>(k), static_cast<int>(j));
                    }
                    T tau_dot = tau(i) * dot;
                    mat(static_cast<int>(i), static_cast<int>(j)) -= tau_dot;
                    for (size_t k = i + 1; k < Rows; ++k) {
                        mat(static_cast<int>(k), static_cast<int>(j)) -= tau_dot * mat(static_cast<int>(k), static_cast<int>(i));
                    }
                }
            }
            return MathStatus::SUCCESS;
