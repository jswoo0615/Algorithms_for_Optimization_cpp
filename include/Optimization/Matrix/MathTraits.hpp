...
struct MathTraits<Optimization::DualVec<T, N>> {
    static Optimization::DualVec<T, N> abs(const Optimization::DualVec<T, N>& x) {
        return Optimization::ad::abs(x);
    }
    static Optimization::DualVec<T, N> sqrt(const Optimization::DualVec<T, N>& x) {
        return Optimization::ad::sqrt(x);
    }
    static Optimization::DualVec<T, N> max(const Optimization::DualVec<T, N>& x,
                                           const Optimization::DualVec<T, N>& y) {
        return (x.v > y.v) ? x : y;
    }
    static Optimization::DualVec<T, N> min(const Optimization::DualVec<T, N>& x,
                                           const Optimization::DualVec<T, N>& y) {
        return (x.v < y.v) ? x : y;
    }
    static bool near_zero(const Optimization::DualVec<T, N>& x,
                          T tol = std::numeric_limits<T>::epsilon() * static_cast<T>(10.0)) {
        return std::abs(x.v) <= tol;
    }
