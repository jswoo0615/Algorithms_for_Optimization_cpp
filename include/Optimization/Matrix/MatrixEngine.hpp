 ...
 * 
 * 선형 대수학에서 수학적 벡터(Column Vector, 열 벡터)는 본질적으로 N행 1열(N x 1) 형태의 행렬입니다.
 * 따라서 벡터를 위한 별도의 클래스를 구현하지 않고, `StaticMatrix<T, N, 1>`을 
...
struct MathTraits {
     ...
     * 
     * [수치적 안정성 판단]
     * 선형 대수 분해(LU, Cholesky, LDLT 등) 과정에서 피벗(Pivot, 대각 원소)으로 나누는 연산이 빈번합니다.
     * 이때 피벗이 수학적 0은 아니더라도 0에 극도로 가까운 수치적 노이즈라면(특이 행렬, Singular),
     * 나눗셈 결과가 무한대(Inf)로 발산하여 시스템이 붕괴합니다.
     * 따라서 `std::numeric_limits<T>::epsilon()`을 사용하여 하드웨어 부동소수점 아키텍처에 맞는 
     * 최소 허용 오차 범위 내에 값이 존재하는지(사실상 0인지) 안전하게 판단합니다.
     */
...
struct MathTraits<Optimization::Dual<T>> {
     ...
     * '미분값(gradient/hessian)'이 아니라 오직 순수 '함숫값(Value, 실수부)'에 의해서만 결정되어야 합니다.
     * 따라서 Dual 구조체 전체의 크기를 비교하는 것이 아니라, 내부의 실수부 멤버인 `v`만 추출하여 입실론과 비교합니다.
     */
 ...
 * `std::array`와 같은 정적 메모리(Stack 영역 또는 전역 BSS 영역)를 기반으로 설계된 행렬 클래스입니다.
 * 이는 메모리 할당/해제에 따른 예측 불가능한 지연(Latency)을 제거하여 NMPC(비선형 모델 예측 제어)와 같은 
 * Hard Real-time(강성 실시간) 시스템의 데드라인을 엄격히 보장하며, 장기 구동 시 발생하는 Heap 단편화(Fragmentation)를 원천 차단합니다.
 *
 ...
 *           + 블록 조립 연산 (거대한 KKT 최적성 조건 행렬과 같이 크고 희소한/블록 형태의 행렬을 빠르게 조립하기 위한 기능)
 */
template <typename T, size_t Rows, size_t Cols>
class StaticMatrix {
   private:
    ...
    // alignas(64): 최신 CPU(x86_64, ARM 등)의 L1 캐시 라인(Cache Line) 크기인 64바이트 단위로 메모리 시작 주소를 정렬합니다.
    //   - 캐시 히트율(Cache Hit Rate) 극대화: 프로세서가 메모리를 읽을 때 64바이트 덩어리(청크) 단위로 가져오므로, 정렬이 어긋나서 캐시 라인을 두 번 읽는 패널티(False Sharing 등)를 방지합니다.
    //   - SIMD 가속 호환: AVX-512 등의 벡터화(Vectorization) 명령어는 메모리가 정렬되어 있지 않으면 심각한 성능 저하나 하드웨어 예외를 발생시킵니다.
    //   - 이 배열은 Column-major(열 우선) 방식이므로, `data` 배열 전체가 메모리 상에 완벽하게 일렬로 연속된(Contiguous) 하나의 1차원 블록을 형성합니다.
    // ============================================================
   ...
   public:
    ...
    // 메모리 주소 계산 공식: `data[col * Rows + row]`
    // 즉, 같은 열(Column)에 있는 원소들이 메모리 상에 연속적으로 배치되어 있습니다.
    // 열 단위의 연산(예: 행렬 곱셈, 블록 삽입/추출) 시 공간 지역성을 극대화하여 캐시 미스(Cache Miss)를 획기적으로 줄입니다.
    // ============================================================
    ...
    // 행렬을 마치 1차원 긴 배열처럼 다룰 때 사용합니다. 요소별 덧셈/뺄셈 연산이나 블록 복사 시 포인터 산술 연산을 위해 제공.
    T& operator()(size_t i) {
        assert(i < Rows * Cols);
        return data[i];
    }
     ...
     * 최신 C++ 최적화 컴파일러가 이를 감지하고 단일 어셈블리 명령어인 고속 `memset`(메모리 블록 채우기) 수준으로 
     * 치환(Intrinsic)하여 연산 사이클을 극한으로 단축합니다.
     */
    ...
    // 1차원 선형 배열(data)을 0번 인덱스부터 끝까지 한 번에 쭉 순회(Linear Scan)하여 루프 오버헤드를 최소화합니다.
    // ============================================================
    ...
    StaticMatrix<T, Rows, Cols> operator/(T scalar) const {
        // AD(자동 미분) 호환 Epsilon 검사를 통해 나누려는 스칼라 값이 0에 너무 가깝지는 않은지 확인합니다.
        // 0으로 나누기(Divide by Zero) 시도 시 프로그램이 즉각 종료(Crash)되는 것을 막고 예외 처리(Exception)로 유도.
        ...
        // [연산 최적화 팁]
        // CPU 명령어 사이클 측면에서 부동소수점 나눗셈(FDIV)은 곱셈(FMUL)에 비해 약 10~20배 정도 압도적으로 느립니다.
        // 루프 내에서 N*M 번 나누는 대신, 먼저 스칼라의 역수(Inverse)를 단 1번만 구하고, 
        // 루프 안에서는 역수를 곱해주는 방식으로 변환하여 엄청난 속도 이득을 얻습니다.
     ...
     *
     * [이론적 배경 및 최적화 원리]
     * 일반적인 학부 수준의 행렬 곱셈(C = A * B) 루프 순서는 행(i) -> 열(j) -> 내적(k) 즉, "I-J-K" 순서입니다.
     * 그러나 이 클래스는 Column-major(열 우선) 메모리 레이아웃을 취하고 있습니다.
     * I-J-K 순서로 연산할 경우, 행렬 B의 요소는 열(j)을 따라가므로 메모리를 연속적으로 예쁘게 읽지만(Stride 1),
     * 행렬 A의 요소는 같은 행(i)의 다음 열(k) 요소로 넘어가기 위해 메모리 주소를 매번 건너뛰면서(Stride가 Rows 크기) 읽게 됩니다.
     * 이는 CPU 캐시에 담겨있지 않은 엉뚱한 주소를 계속 요구하므로 막대한 **캐시 미스(Cache Miss)** 패널티를 유발하여 성능을 나락으로 떨어뜨립니다.
     *
     * 이를 완벽히 방지하기 위해 루프의 순서를 "J-K-I" 순서로 재배치(Loop Reordering)했습니다.
     * 이 J-K-I 구조에서는 가장 빈번하게 도는 최하단(가장 안쪽) I-루프가 
     * Result 행렬의 특정 열(res_col)과, 행렬 A의 특정 열(a_col)을 위에서 아래로(행 i를 증가시키며) 순차적으로 쭉 훑고 지나갑니다.
     * 이 형태는 메모리에 완전히 일렬로 순차 접근(Sequential Access)하는 형태이므로 캐시 적중률이 100%에 가까워지며,
     * 컴파일러가 SIMD 벡터화(AVX, SSE) 인스트럭션을 적용하여 루프를 병렬로 묶어 처리하기 가장 이상적인 형태가 됩니다.
     *
    ...
    StaticMatrix<T, Rows, OtherCols> operator*(
        ...
        for (size_t j = 0; j < OtherCols; ++j) {
            T* res_col = res_base + (j * Rows);  // Result 행렬의 j번째 열이 시작되는 메모리 주소를 캐싱 (포인터 산술)
            
            // K 루프: 행렬 A의 열(Column) 이동 및 행렬 B의 행(Row) 이동 (내적의 덧셈 항)
            for (size_t k = 0; k < Cols; ++k) {
                const T* a_col = a_base + (k * Rows);  // 행렬 A의 k번째 열 시작 주소 캐싱
                const T b_kj = Other(static_cast<int>(k), static_cast<int>(j));  // I 루프 동안 곱해질 B의 스칼라 값 고정
                
                // I 루프 (가장 안쪽): Result의 열과 A의 열을 아래 방향으로 순차적으로 연속 접근
                // 최적화 컴파일러는 이 루프를 감지하고 레지스터 FMA(Fused Multiply-Add) 명령어로 치환합니다.
    ...
    StaticMatrix<T, Cols, Rows> transpose() const {
        ...
        for (size_t j = 0; j < Cols; ++j) {
            ...
            for (size_t i = 0; i < Rows; ++i) {
                // 대상 Result 행렬은 (j, i) 순으로 접근되어 메모리 점프가 발생하지만, 전치의 특성상 불가피합니다.
    ...
    // 
    // [솔버 설계의 필요성]
    // 모델 예측 제어(MPC)의 핵심은 매 주기마다 거대한 KKT 형태의 선형 연립 방정식 시스템(Ax = b)을 푸는 것입니다.
    // 수학적으로 x = A^{-1} * b 이지만, 컴퓨터로 직접 A의 역행렬(Inverse)을 통째로 구하는 것은
    // O(N^3)의 막대한 연산량이 소모되며 무엇보다 부동소수점 오차가 기하급수적으로 누적되어 시스템이 불안정해집니다(수치적 불안정성).
    // 따라서 현대 수치해석에서는 반드시 행렬 A를 다루기 쉬운 두세 개의 삼각 행렬 조각으로 "분해(Decomposition)"한 뒤,
    // 전진 대입(Forward Substitution)과 후진 대입(Backward Substitution)이라는 아주 싼 연산(O(N^2))을 통해 빠르고 정확하게 해(x)를 구합니다.
    // ============================================================

    /**
     * @brief LU 분해 (In-place Doolittle 알고리즘)
     *
     * 모든 정방 행렬(Square Matrix) A를 단위 하삼각행렬 L(Lower)과 상삼각행렬 U(Upper)의 곱 (A = L * U)으로 쪼갭니다.
     * Doolittle 알고리즘을 사용하며, 메모리를 아끼기 위해(제한된 임베디드 자원 고려) 추가적인 L, U 행렬 공간을 만들지 않고 
     * 원본 행렬 A의 메모리 공간에 분해된 결과값들을 그대로 덮어씁니다(In-place 방식).
     * (참고: L 행렬의 대각 성분(Pivot)은 무조건 1.0이므로 저장 공간을 생략할 수 있습니다)
     *
     * @return true  - 분해가 완벽히 성공함.
     * @return false - 계산 도중 특정 대각 원소(Pivot)가 0에 너무 가까워 나눗셈을 할 수 없음 (행렬이 특이성(Singularity)을 가짐을 의미).
     */
    bool LU_decompose() {
        ...
        for (size_t i = 0; i < Rows; ++i) {
            ...
            // 이 값이 0이면 하삼각행렬 L을 구할 때 분모가 0이 되어 무한대(NaN/Inf) 폭발이 발생합니다.
     ...
     * 
     * 위에서 LU_decompose()를 성공적으로 마친 상태(A 공간에 L과 U가 겹쳐 저장됨)에서, 시스템 방정식 Ax = b (즉, LUx = b) 의 해 x를 구합니다.
     * 
     * [2단계 해법 알고리즘]
     * 1단계: L(하삼각행렬) * y = b 를 풉니다. 위에서부터 아래로 미지수를 순차적으로 찾기 쉬우므로 전진 대입(Forward Substitution)이라 부릅니다.
     * 2단계: U(상삼각행렬) * x = y 를 풉니다. 밑에서부터 위로 거꾸로 올라가며 미지수를 찾아 해 x를 확정하므로 후진 대입(Backward Substitution)이라 부릅니다.
     * 
    ...
    StaticVector<T, Rows> LU_solve(const StaticVector<T, Rows>& b) const {
        ...
        for (size_t i = 0; i < Rows; ++i) {
            ...
            for (size_t j = 0; j < i; ++j) {
                sum += (*this)(static_cast<int>(i), static_cast<int>(j)) * y(j); // 하삼각 영역(L) 접근
            }
            // Doolittle LU에서 L의 대각 원소는 암묵적으로 항상 1.0 이므로, 나눗셈 처리가 아예 생략되어 연산이 매우 빠릅니다.
            y(i) = b(i) - sum;  
        }
        ...
        for (int i = static_cast<int>(Rows) - 1; i >= 0; --i) {
            ...
            // U의 대각 성분은 1이 아니므로, 방금 구한 y값에서 sum을 빼고 U의 대각 원소로 나누어 최종 x를 구합니다.
     ...
     *
     * [제약 조건 및 강점]
     * 대상 행렬 A가 반드시 대칭 행렬(Symmetric, A = A^T)이고, 동시에 양의 정부호(Positive Definite, 모든 고윳값이 양수)일 때만 수학적으로 성립하는 특수 분해 기법입니다.
     * 일반 LU 분해에 비해 필요한 반복 연산량이 정확히 절반(O(N^3/3))으로 줄어들며, 부동소수점 오차에 대한 수치적 안정성(Numerical Stability)이 압도적으로 좋습니다.
     * SQP 최적화나 Gauss-Newton (J^T J 구조) 형태의 역학/제어 문제를 풀 때, Hessian 행렬은 기본적으로 양의 정부호 대칭성을 띠므로 이때 가장 강력한 최우선 솔버로 활용됩니다.
     * 분해 결과인 L 행렬은 하삼각 공간에 덮어써서 저장되며 상삼각 영역은 버려집니다.
     *
     * @return true  - A가 양의 정부호 행렬임이 증명되어 분해가 정상 성공함.
     * @return false - A가 비양정치 행렬(Non-Positive Definite)이거나 음수 고윳값이 발생함. (수식 내부에서 루트 안에 음수가 들어가려 할 때 즉각 발각되어 차단됨).
     */
    bool Cholesky_decompose() {
        ...
        for (size_t j = 0; j < Cols; ++j) {
            ...
            // 0 이하거나 입실론 이하의 수치적 찌꺼기라면, 행렬이 볼록성(Convexity)을 잃었거나 특이행렬이 된 것이므로
            // 치명적인 허수(Imaginary number) 발생을 막기 위해 무조건 실패(false)를 반환하고 즉시 탈출합니다.
     ...
     * 
     * LU 해법과 동일하게 전진-후진 대입을 사용하나, 상삼각행렬 U 대신 L의 전치행렬인 L^T를 사용합니다.
     */
    StaticVector<T, Rows> Cholesky_solve(const StaticVector<T, Rows>& b) const {
        ...
        // 메모리에 접근할 때 원본 L 행렬의 인덱스 접근 순서(행과 열)를 뒤집어서 읽어오는( (*this)(k, i) ) 영리한 트릭을 사용합니다.
        for (int i = static_cast<int>(Rows) - 1; i >= 0; --i) {
            T sum = static_cast<T>(0);
     ...
     *
     * [등장 배경]
     * Cholesky 분해는 매 열마다 무거운 무리수 루트(sqrt) 연산을 강제합니다. CPU 성능이 매우 제한된 구형 임베디드 마이크로컨트롤러에서는 이는 병목이 될 수 있습니다.
     * 루트 연산을 수학적으로 우회하여 제거하기 위해 대각 행렬 D를 중앙에 끼워넣는 방식으로 변형한 것이 LDLT 분해입니다.
     * 
     * 이론상 행렬이 0의 고윳값을 갖는 준정부호(Positive Semi-definite) 행렬이더라도 분해가 가능하도록 설계되었으나,
     * 본 엔진은 제어 솔버(NMPC, SQP) 내부망에 통합되어 있으므로 알고리즘의 붕괴를 사전에 막기 위해 D의 원소가 엄격한 양수(Positive Definite)인지 입실론 수준에서 강하게 검열합니다.
     * 하삼각 영역에 L을, 대각선 영역에 D 원소를 동시에 덮어써서 콤팩트하게 저장합니다. (이때 L의 대각은 암묵적 1로 처리됨)
     *
     * @return true  - 분해 성공
    ...
    bool LDLT_decompose() {
        ...
        for (size_t j = 0; j < Cols; ++j) {
            ...
            for (size_t k = 0; k < j; ++k) {
                ...
                // L의 원소 제곱에 대각 원소 D_kk를 스케일링 팩터로 곱하여 누산 (루트 대신 사용되는 핵심 트릭)
            ...
            (*this)(static_cast<int>(j), static_cast<int>(j)) = D_jj;  // 대각 원소 자리에 D 성분 저장
            ...
            for (size_t i = j + 1; i < Rows; ++i) {
                ...
                for (size_t k = 0; k < j; ++k) {
                    sum_L +=
                        ...
                        (*this)(static_cast<int>(j), static_cast<int>(k)) *
                        (*this)(static_cast<int>(k), static_cast<int>(k));  // 공식 전개: L_ik * L_jk * D_kk
                }
                (*this)(static_cast<int>(i), static_cast<int>(j)) =
                    ((*this)(static_cast<int>(i), static_cast<int>(j)) - sum_L) /
                    (*this)(static_cast<int>(j), static_cast<int>(j));  // 확정된 D_jj로 나누어 마무리
            }
        }
        return true;
    }
    ...
    StaticVector<T, Rows> LDLT_solve(const StaticVector<T, Rows>& b) const {
        ...
        // 이 한 줄의 나눗셈 과정이 Cholesky 분해의 무거운 무리수 루트(sqrt) 연산을 훌륭하게 대체하는 수학적 역할을 합니다.
        for (size_t i = 0; i < Rows; ++i) {
     ...
     *
     * 정방 행렬뿐만 아니라 행이 더 많은 직사각형 행렬(Over-determined System)도 처리할 수 있는 직교(Orthogonal) 기반 분해 솔버입니다.
     * 행렬 A의 열 벡터들을 순차적으로 정규화(Normalize)하고 직교화(Orthogonalize)하여 직교 행렬 Q와 상삼각 행렬 R을 만들어냅니다.
     * 
     * [MGS(Modified)의 핵심 최적화]
     * 학부 수준의 고전적인 CGS(Classical Gram-Schmidt) 알고리즘은 계산 중 부동소수점 오차가 기하급수적으로 쌓여 
     * 최종 도출된 Q 벡터들이 직교성을 상실하고 한쪽으로 무너지는 끔찍한 단점이 있습니다.
     * MGS 알고리즘은 이를 방지하기 위해, 하나의 직교 Q 열을 찾아낼 때마다 그 직교성을 "아직 처리하지 않은 모든 나머지 열들에서"
     * 즉각적으로 깎아내고 빼버리는(투영 소거, Projection) 전략을 사용하여, 컴퓨터 수치 환경에서 훨씬 강건하고 완벽한 직교 기반을 유지합니다.
     * Q 행렬은 원본 A 공간(*this)에 덮어써지고, R 행렬은 별도의 외부 변수를 통해 반환받습니다.
     *
     * @param R (출력용) 분해된 상삼각행렬 R을 담을 외부 행렬 객체 
     * @return true  - 분해 성공
     * @return false - 특정 열이 이전 열 벡터들과 선형 종속(Linear Dependent) 관계여서 직교 기반 생성이 불가능할 경우 
     */
    bool QR_decompose_MGS(StaticMatrix<T, Cols, Cols>& R) {
        ...
        for (size_t i = 0; i < Cols; ++i) {
            ...
            // 열 벡터의 길이가 사실상 0 이라면, 선형 종속성(Linear Dependency)으로 인해 직교 기저(Orthogonal basis)를 형성할 수 없으므로 포기합니다.
            if (MathTraits<T>::near_zero(R(static_cast<int>(i), static_cast<int>(i)))) {
                return false;
            }

            // 2. 현재 열 벡터를 앞서 구한 길이 Norm으로 나누어 크기가 1인 단위 벡터(정규화) Q_i로 변환 (원본 행렬 공간 덮어쓰기)
            for (size_t k = 0; k < Rows; ++k) {
                (*this)(static_cast<int>(k), static_cast<int>(i)) /=
            ...
            // 이제 막 확정된 정규 단위 벡터 Q_i 를 기준으로, 그보다 오른쪽에 있는 모든 열(j)들에 대해
            // Q_i 방향으로 겹쳐진 그림자 성분(성분 길이 = 내적 dot)을 계산하고, 그 그림자 성분을 완전히 빼버려 독립적으로 만듭니다.
     ...
     * 
     * Q 행렬은 직교 행렬(Orthogonal Matrix)이므로 놀랍게도 Q의 역행렬(Q^-1)은 단순히 그 전치 행렬(Q^T)과 정확히 동일합니다.
     * 따라서 양변에 Q^T 를 곱하면 좌항은 Rx 만 깔끔하게 남게 되어 초고속 역산이 가능합니다.
     */
     ...
     * MGS 알고리즘은 성능이 뛰어나지만 정밀도가 극단적으로 요구되는 제약조건 KKT 시스템(Jacobian의 널스페이스(Null-space)를 도출할 때 등)에서는 간혹 직교성이 깨지기도 합니다.
     * 반면 하우스홀더 QR 알고리즘은 거울처럼 빛을 반사시키는 물리적 기하학(Reflection) 개념을 이용해 벡터를 특정 초평면(Hyperplane)에 접어 대칭 이동시킵니다.
     * 이 방식은 컴퓨터 부동소수점 오차가 발생하더라도 직교성이 구조상 절대 무너지지 않아 가장 완벽한 해상도를 보장합니다.
     * 
     * [Compact Storage 최적화 마법]
     * 더 놀라운 점은 행렬 Q 전체를 명시적으로 N*N 크기만큼 메모리에 만들고 저장하지 않는다는 것입니다.
     * 거울의 각도 방향을 결정하는 고유한 Householder 반사 벡터 v 만을 추출하여, 원본 행렬 A의 
     * 쓸모없어진 하삼각 영역 빈 공간에 차곡차곡 구겨 넣어 콤팩트하게 저장(Compact Storage)합니다. 
     * 이 천재적인 최적화로 인해 메모리 사용량이 극적으로 절약되며 거대한 행렬 분해에 최적입니다.
     *
     * @param tau  거울 반사 스케일링 팩터(Scaling factor) 값들을 저장해 둘 별도의 추가 벡터 
     * @return true (하우스홀더는 웬만한 특이 열이 나타나도 tau값을 0으로 닫아 스킵하며 알고리즘 자체가 무너지지 않고 무조건 계속 굴러가는 안정성을 자랑함)
     */
    bool QR_decompose_Householder(StaticVector<T, Cols>& tau) {
        ...
        for (size_t i = 0; i < Cols; ++i) {
            ...
            // 부분 벡터 전체가 완벽하게 0벡터에 가깝다면, 반사(Reflection) 액션을 수행할 필요 없이 스킵.
            ...
            // 부호 처리 (Sign Handling)
            // 컴퓨터 수학의 고질적 문제인 수치 소거 오류(Cancellation Error, 비슷한 크기의 부동소수점을 뺄 때 유효 숫자가 뭉텅이로 날아가는 치명적 현상)를 방지하기 위한 안전장치입니다.
            // 기존 대각 원소의 부호와 완전히 반대 부호를 취하여 뺄셈이 덧셈처럼 작용하도록 만듭니다.
            ...
            // 현재 열보다 우측에 위치한 모든 열 벡터(A 행렬의 뒷부분)들을 전부 이 거울에 통과시켜 각도를 반사시켜 접어버립니다. (A <- (I - tau*v*v^T) A)
            for (size_t j = i + 1; j < Cols; ++j) {
                T dot = (*this)(static_cast<int>(i), static_cast<int>(j));  // v_0 = 1 기준 성분 초기화
                for (size_t k = i + 1; k < Rows; ++k) {
                    dot += (*this)(static_cast<int>(k), static_cast<int>(i)) *
                           (*this)(static_cast<int>(k), static_cast<int>(j));
                }
     ...
     * 하삼각 부분에 조각조각 구겨 넣어 숨겨둔 반사 벡터(v) 조각들과 tau 스케일 배열을 차례로 재조립하여,
     * 우항 벡터 b 에 대해 도미노 쓰러뜨리듯 연속적으로 직교 거울 반사를 적용하면 
     * 자연스럽게 좌항의 (Q^T * b) 연산 결과 벡터가 튀어나오게 되는 고도로 최적화된 테크닉을 사용합니다.
     */
    StaticVector<T, Cols> QR_solve_Householder(const StaticVector<T, Cols>& tau,
                                               const StaticVector<T, Rows>& b) const {
        StaticVector<T, Rows> y = b;

        // 1단계: b 벡터에 대해 순차적 거울 반사(Sequential Reflection) 시뮬레이션 적용 (Q^T b 계산 효과)
        for (size_t i = 0; i < Cols; ++i) {
        ...
        // 2단계: 후진 대입 (Back substitution: R * x = y)
        // R 행렬의 데이터는 원본 배열의 상삼각 영역(Upper Triangular)에 고스란히 남아있으므로 이를 활용.
    ...
    // 최적성 조건을 나타내는 거대한 KKT 행렬을 매 주기마다 새롭게 퍼즐 조립하듯 끼워 맞춰 생성해야 합니다.
    // (예: 좌상단에 목적 함수 Hessian 덩어리, 좌하단 및 우상단에 제약 조건 A 덩어리 등)
    // 이 유틸리티들은 "Column-major 특성상 같은 열(Column)은 메모리 연속이다" 라는 핵심 사실을
     ...
     * 한 열(Column) 덩어리 전체가 메모리에 일렬 주소로 연속해 늘어서 있다는 물리적 성질을 악용합니다.
     * C++ 표준 라이브러리인 `std::copy`를 사용하면, 이 함수가 컴파일될 때 내부적으로 CPU 블록 카피 명령(memcpy 등)으로 치환되어
     * 한 번 호출 시 1개의 열 블록 전체가 압도적인 속도로 단번에 타겟 영역에 복사되어 박힙니다.
     * 
    ...
    void insert_block(size_t start_row, size_t start_col,
        ...
        // [컴파일 타임 에러 검출] 템플릿 크기 자체가 타겟 행렬의 허용 크기를 초과하면 애초에 빌드를 차단
     ...
     * (이 경우는 메모리 구조가 뒤틀리기 때문에 std::copy 선형 복사 최적화 마법을 쓸 수 없고, 캐시 흩뿌림(Scattering) 방식의 2중 루프가 불가피합니다)
     */
    template <size_t SubRows, size_t SubCols>
    void insert_transposed_block(size_t start_row, size_t start_col,
        ...
        for (size_t j = 0; j < SubCols; ++j) {
            for (size_t i = 0; i < SubRows; ++i) {
                // 타겟(this)에는 (j, i) 순으로 좌표를 뒤집어서 박아넣어 물리적인 전치(Transpose) 효과를 부여함
     ...
     *
     * KKT 시스템과 같은 초대형 복합 선형 방정식을 풀고 나면 엄청나게 긴 형태의 결과 벡터를 얻게 됩니다.
     * 이 벡터에는 상태 변수(x), 슬랙 변수(s), 듀얼 승수(lambda) 등이 잡탕으로 섞여 있습니다.
     * 제어 로직에서는 이 중에서 로봇/차량에 실질적인 지시를 내릴 '제어 입력 신호(Control Input, u)' 블록만 깔끔하게 도려내야 합니다.
     * `insert_block`과 정반대 순서로, 완벽히 동일한 메모리 고속 복사(std::copy) 최적화 기법을 적용하여 덩어리째 뜯어옵니다.
     */
    template <size_t SubRows, size_t SubCols>
    ...
    // 원인이 되는 KKT 시스템 매트릭스가 제대로 조립되었는지 콘솔 화면에서 시각적으로 직접 확인하기 위한 필수 함수입니다.
    // ============================================================
    void print(const char* name) const {
        ...
        for (size_t i = 0; i < Rows; ++i) {
            for (size_t j = 0; j < Cols; ++j) {
                // iomanip 라이브러리를 이용하여 정해진 폭(10칸)과 고정된 소수점 자리수(4자리)를 맞춰 표처럼 깔끔하게 출력
