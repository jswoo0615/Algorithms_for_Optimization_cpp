...
class StaticSparseMatrix {
   public:
    ...
    /** 
     * @brief 각 행(Row)의 첫 번째 0이 아닌 원소가 `values` 배열에서 어느 위치부터 시작하는지 나타내는 배열.
     * 크기는 (Rows + 1)이며, 마지막 원소는 총 NNZ(Non-Zero) 개수를 나타냅니다.
     */
     ...
     * 
     * 주의: 이 함수를 호출할 때는 행(Row) 순서대로(위에서 아래로, 왼쪽에서 오른쪽으로) 추가하는 것이 좋습니다.
     * 이 함수를 모두 호출한 후에는 반드시 `finalize()`를 호출하여 `row_ptr`를 올바른 CSR 형식으로 완성해야 합니다.
     */
     ...
     * 희소 행렬 요소 추가가 모두 끝난 후 곱셈 등의 연산을 수행하기 전에 반드시 한 번 호출해야 합니다.
     */
    void finalize() {
    ...
    void multiply_transpose(const StaticVector<T, Rows>& x, StaticVector<T, Cols>& y) const {
        ...
        for (size_t i = 0; i < Rows; ++i) {
            ...
            for (int j = row_ptr(i); j < row_ptr(i + 1); ++j) {
                // A(i, j) * x(i) 는 y(j)에 더해집니다. (A^T의 관점에서는 A^T(j, i) * x(i)가 되기 때문)
