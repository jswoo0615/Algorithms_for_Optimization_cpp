 ...
 * 지역 최솟값(Local Minima)을 탈출하거나 평탄한 지역(Plateau)을 빠르게 지나가기 위해 고안되었습니다.
 * 물리적인 '관성(Momentum)'의 개념을 차용하여, 과거에 이동했던 방향(속도, Velocity)을 
 * 기억해두고 현재의 기울기(Gradient) 업데이트에 일정 비율(beta)만큼 더해주는 방식입니다.
 * 
 * 수식:
...
class Momentum {
   public:
     ...
     * @param alpha 학습률 (보폭, Learning Rate). 현재 기울기를 얼마나 반영할지 결정합니다. (기본값: 0.001)
     * @param beta 관성 계수 (Momentum). 과거 속도를 얼마나 유지할지 결정합니다. (일반적으로 0.9 근처의 값을 사용)
    ...
    static std::array<double, N> optimize(Func f, std::array<double, N> x, double alpha = 0.001,
        ...
        std::array<double, N> v = {0.0};  // 속도(Velocity) 누적 변수. 초기 상태는 정지(0.0) 상태로 시작.
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            // 기울기 벡터의 크기(L2-Norm)를 계산하여 현재 위치가 평탄한지(기울기가 0에 가까운지) 확인합니다.
            double g_norm = 0.0;
            for (size_t i = 0; i < N; ++i) g_norm += g[i] * g[i];
            g_norm = std::sqrt(g_norm);

            // 기울기 노름이 허용 오차(tol)보다 작으면 최적점(임계점)에 도달한 것으로 판단하고 조기 종료합니다.
            if (g_norm < tol) {
            ...
            for (size_t i = 0; i < N; ++i) {
                // 이전 속도(v)에 관성 계수(beta)를 곱하여 이전 이동 방향을 유지하려는 성질을 부여하고,
                // 현재 기울기(g)에 학습률(alpha)을 곱한 값을 빼주어(경사 하강) 새로운 이동 속도(방향 및 크기)를 결정합니다.
