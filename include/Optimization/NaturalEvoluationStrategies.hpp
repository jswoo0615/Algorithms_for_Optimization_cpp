 ...
 * 일반적인 경사 하강법과 달리, 파라미터 공간의 기하학적 구조를 반영하는 피셔 정보 행렬(Fisher
 * Information Matrix)의 역행렬을 곱한 '자연 기울기(Natural Gradient)'를 사용하여 업데이트 방향을
 * 결정합니다.
 *
 * @note 본 구현체는 다음과 같은 특징을 가집니다:
 * 1. **Natural Gradient**: 피셔 정보 행렬 역행렬을 적용하여 바닐라 기울기가 가지는 스케일 및 나눗셈
 * 폭탄(Division by zero) 문제를 해소하고 안전한 업데이트를 보장합니다.
 * 2. **Fitness Normalization (Z-Score)**: 목적 함수의 절대적인 스케일 차이에 의해 기울기가
 * 폭주(Overshooting)하는 현상을 막기 위해 평가된 함수값들을 정규화합니다.
 */
class NaturalEvolutionStrategies {
   public:
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> mu_init,
        ...
        static thread_local std::array<std::array<double, N>, M>
            z_cache;  // 정규 분포에서 뽑은 노이즈 z 저장
        static thread_local std::array<double, M>
            y_cache;  // 각 샘플 위치에서의 함수 평가값 f(x) 저장
...
#pragma omp simd
                for (size_t i = 0; i < N; ++i) {
                    ...
                    // 일반적인 Log-Likelihood의 기울기(Vanilla Gradient)에 Fisher Information
                    // Matrix의 역행렬(F^{-1})을 곱한 결과입니다. 놀랍게도 복잡한 나눗셈 항들이
                    // 상쇄되어 매우 우아하고 단순한 곱셈 형태가 됩니다.
