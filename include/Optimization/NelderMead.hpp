 ...
 *
 * @details Nelder-Mead 알고리즘은 함수의 미분 정보(기울기)를 사용하지 않는 Derivative-Free
 * Optimization(DFO) 기법입니다. N차원 공간에서 N+1개의 정점(Vertex)으로 이루어진 다면체인
 * '심플렉스(Simplex)'를 구성하고, 함수값이 가장 나쁜(Worst) 정점을 중심점(Centroid)을 기준으로
 * 반대편으로 넘기는(Reflection) 등의 기하학적 변형을 반복하며 마치 '아메바(Amoeba)'가 기어가듯
 * 최솟값을 향해 이동합니다.
 *
 * 주요 연산:
 ...
 * 1. **MISRA C++ 준수 및 O(1) 동적 할당 제로**: std::vector 대신 std::array를 사용하여 힙(Heap)
 * 메모리 할당 오버헤드 제거
 * 2. **SIMD / FMA 가속**: #pragma omp simd 및 std::fma를 적극 활용하여 벡터 연산 및 분산 계산 속도
 * 극대화
 */
class NelderMead {
   private:
    ...
    struct Vertex {
        ...
        // 정점들을 함수값 기준으로 오름차순 정렬하기 위한 연산자 오버로딩 (f_val이 작을수록 좋은
        // 점)
   ...
   public:
     ...
     * @param tol 허용 오차. 심플렉스 정점들의 함수값 분산(Variance)이 이 값의 제곱보다 작아지면
     * 수렴한 것으로 판단 (기본값: 1e-6)
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> x_start,
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // N+1개의 함수값들의 분산을 계산하여, 이 분산이 허용 오차의 제곱(tol_sq) 미만이면
            // 수렴한 것으로 판단
            double mean_y = 0.0;
#pragma omp simd
            for (size_t i = 0; i <= N; ++i) {
...
#pragma omp simd
            ...
            // 반사된 점(x_r)이 최고점(Best)보다는 나쁘지만 차악점(Second Worst, N-1)보다는 좋을
            // 경우 평범하게 반사된 점을 수용함
...
#pragma omp simd
            ...
            // 반사된 점이 차악점(Second Worst)보다도 나쁜 경우 (해당 방향이 아니거나 계곡임)
                ...
                // 그래도 최악의 점(Worst)보다는 반사된 점이 나은 경우 -> 외부 수축 (Outside
                // Contraction)
...
#pragma omp simd
                ...
                // 8. 축소 (Shrinkage)
                // 수축(Contraction)으로도 나은 점을 찾지 못한 경우, 심플렉스가 골짜기에 꼈다고
                // 판단하고 최우수 점(simplex[0])을 제외한 모든 점들을 최우수 점 방향으로 일정
                // 비율(sigma) 당김
                if (do_shrink) {
