 ...
 *
 * @details 일반적인 모멘텀(Momentum) 방식은 현재 위치에서 기울기를 계산한 뒤 속도(Velocity)에
 * 더하여 이동합니다. 반면, 네스테로프 모멘텀(NAG) 방식은 현재 지니고 있는 관성(Velocity)을 이용해
 * 미리 미래 위치(Look-ahead)를 예상하고, 그 "예상된 미래 위치"에서의 기울기를 계산하여 현재 속도를
 * 업데이트합니다.
 *
   ...
   public:
    ...
    static std::array<double, N> optimize(Func f, std::array<double, N> x, double alpha = 0.001,
        ...
        for (size_t iter = 1; iter <= max_iter; ++iter) {
            ...
            // 이 기울기는 "앞으로 내리막이 계속될지, 오르막으로 변할지"에 대한 정보를 담고
            // 있습니다.
            // -----------------------------------------------------------
            double f_lookahead;
            ...
            // 미래 위치에서 구한 기울기를 현재 속도 업데이트에 반영하여 실제 이동할 위치를
            // 계산합니다.
            // -----------------------------------------------------------
            for (size_t i = 0; i < N; ++i) {
