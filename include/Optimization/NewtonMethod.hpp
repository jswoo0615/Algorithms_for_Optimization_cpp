 ...
 *
 * @details 뉴턴 메서드는 목적 함수의 2차 테일러 근사(Taylor Approximation)를 활용하여 최솟값을 찾는
 * 방법입니다. 1차 미분인 기울기(Gradient, g)와 2차 미분인 헤시안 행렬(Hessian Matrix, H)을 모두
 * 사용하여 최적점까지의 방향과 거리를 한 번에 도출하므로, 수렴 속도(2차 수렴, Quadratic
 * Convergence)가 매우 빠릅니다.
 *
 * 수식:
 * x_{t+1} = x_t - H^{-1} * g
 * (실제 구현에서는 역행렬 H^{-1}을 직접 구하지 않고 선형 방정식 H * p = -g 를 풀어 이동 방향 p를
 * 구합니다.)
 *
...
class NewtonMethod {
   private:
     ...
     * @param g 기울기 벡터 (N차원)
     * @return 방정식의 해(이동 벡터 p). 단, 행렬이 특이(Singular)하여 해를 구할 수 없는 경우
     * std::nullopt 반환
     */
    template <size_t N>
    [[nodiscard]] static constexpr std::optional<std::array<double, N>> solve_newton_system(
        ...
        if constexpr (N == 2) {
            ...
            // 판별식이 0에 매우 가까우면 역행렬이 존재하지 않음 (지형이 완전 평탄하거나
            // 안장점(Saddle Point))
   ...
   public:
     ...
     * @param tol 허용 오차. 기울기의 크기(Norm)가 이 값 미만이 되면 수렴한 것으로 간주 (기본값:
     * 1e-6)
    ...
    [[nodiscard]] static OptimizationResult<N> optimize(Func f, std::array<double, N> x_init,
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // 리포지토리에 자체 구현된 AutoDiff 엔진을 활용하여 O(1) 정적 할당만으로 기울기를
            // 구합니다.
...
#pragma omp simd
            ...
            // 리포지토리의 AutoDiff.hpp에서 제공하는 Central Difference 기반의 헤시안 행렬을
            // 구합니다.
