 ...
 * 평탄한 지역(Plateau), 안장점(Saddle Point), 혹은 얕은 지역 최솟값(Local Minima)에 갇히기
 * 쉽습니다. 노이즈 하강법은 기존의 1차 미분(Gradient) 업데이트 수식에 **무작위 가우시안
 * 노이즈(Gaussian Noise)**를 추가로 주입하여 이러한 함정들을 확률적으로 탈출할 수 있도록 돕는
 * 알고리즘입니다.
 *
 * 핵심은 탐색 초반에는 노이즈를 크게 주어 넓은 영역을 탐색하게 하고,
 * 최적해에 가까워질수록(반복 횟수가 증가할수록) 노이즈의 분산(sigma)을 점진적으로 줄여 정밀하게
 * 수렴하도록 유도하는 것입니다.
 *
 * 수식:
 ...
 * 2. 난수 생성기(mt19937)를 `thread_local`로 선언하여 매 호출 시 발생하는 초기화 오버헤드 원천
 * 차단.
...
class NoisyDescent {
   public:
     ...
     * @param sigma_func 현재 반복 횟수(iter)를 인자로 받아 현재 세대의 표준 편차(sigma)를 반환하는
     * 콜백 함수. (예: `[](size_t iter) { return 1.0 / iter; }` 등 점진적 감소 형태 권장)
     * @param alpha 학습률 (Learning Rate). 경사 하강 보폭 크기 (기본값: 0.01)
     * @param tol 허용 오차. 기울기 크기와 노이즈 크기가 동시에 이 값 미만이 될 때 수렴으로 판단
     * (기본값: 1e-5)
    ...
    [[nodiscard]] static OptimizationResultND<N> optimize(Func f, std::array<double, N> x_start,
        ...
        // 매 이터레이션(반복) 혹은 함수 호출 시마다 엔진을 새로 생성하면 병목(Bottleneck)이
        // 발생하므로 이를 완전 제거
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            // -------------------------------------------------------------
            // 1. 목적 함수값 및 기울기 평가
            // AutoDiff(자동 미분) 엔진을 호출하여 수치적 오차 없는 정확한 함수값과 기울기(g)를
            // 동시에 획득
            // -------------------------------------------------------------
...
#pragma omp simd
            ...
            // 2) 주입되는 노이즈 또한 허용치(tol) 이하로 소멸되어야만 최종 수렴한 것으로
            // 판단합니다.
            // -------------------------------------------------------------
