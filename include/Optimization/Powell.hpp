...
class PowellMethod {
   ...
   public:
     ...
     * @return OptimizationResultND<N> 최적해의 위치, 함숫값, 반복 횟수, 소요 시간을 담은 결과 구조체
     */
...
#pragma omp simd
            ...
            // Powell 방법은 반복하다 보면 새 방향들이 선형 종속적으로 변하여 탐색 공간이 축소될 수 있음.
            // 이를 방지하기 위해 주기적으로 N+1 번째 이터레이션마다 기저 방향 U를 단위 행렬로 초기화시킴.
            if (iter % (N + 1) == 0) {
                reset_basis<N>(U);
            }
        }
