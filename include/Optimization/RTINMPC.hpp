   ...
   public:
    ...
    struct BoundIneq {
        ...
        StaticVector<T, N_ineq> operator()(const StaticVector<T, N_vars>& U) const {
            ...
            for (size_t k = 0; k < Np; ++k) {
                T a = U(static_cast<int>(k * 2));
                T d = U(static_cast<int>(k * 2 + 1));
                ...
                ineq(static_cast<int>(k * 8 + 4)) = x(0) - T(50.0);  // X 상한선
                ineq(static_cast<int>(k * 8 + 5)) = T(-5.0) - x(0);  // X 하한선
                ineq(static_cast<int>(k * 8 + 6)) = x(1) - T(5.0);   // Y 상한선
                ineq(static_cast<int>(k * 8 + 7)) = T(-5.0) - x(1);  // Y 하한선
            }
            return ineq;
        }
    };
    ...
    bool compute_control(const StaticVector<double, Nx>& current_x,
        ...
        for (size_t k = 0; k < Np; ++k) {
            res_f.ref_horizon[k] = ref_horizon[k];
        }
        
        BoundIneq ineq_f{current_x, model, dt};
        
        // [Architect's Action] solve_sparse -> solve_dense 로 강제 변경 (Layer 4 통제)
        bool success = rti.solve_dense(
            U_guess, 
            res_f, 
            [](const auto&) { return StaticVector<double, 0>(); }, 
            ineq_f
        );
        
        // Warm Start를 위한 이전 제어 입력 저장
        if (success) {
            u_last(0) = U_guess(0);
            u_last(1) = U_guess(1);
        }
