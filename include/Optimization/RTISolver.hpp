...
class RTISolver {
   public:
    ...
    bool solve(StaticVector<double, N_vars>& u, ResidualFunc res_f, EqFunc eq_f, IneqFunc ineq_f) {
        ...
        qp_solver.P = H_GN; qp_solver.q = grad_f;
        StaticVector<double, N_vars> p; p.set_zero();
        ...
        if (qp_success) {
            for(size_t i=0; i<N_vars; ++i) {
                ...
                // 최종 물리 한계 클램핑
                if (i % 2 == 0) { if(u(i) > 3.0) u(i)=3.0; if(u(i) < -3.0) u(i)=-3.0; }
                else { if(u(i) > 0.5) u(i)=0.5; if(u(i) < -0.5) u(i)=-0.5; }
            }
            return true;
        } 
        return false; 
