...
class RTISolver {
   public:
    ...
    // ====================================================================
    // 1. Dense Solver (가우스-뉴턴 기반 조밀 행렬 해법)
    // ====================================================================
    ...
    // ====================================================================
    // 2. Sparse Solver (구조적 필터링이 적용된 희소 행렬 해법)
    // ====================================================================
    template <typename ResidualFunc, typename EqFunc, typename IneqFunc>
    bool solve_sparse(StaticVector<double, N_vars>& u, ResidualFunc res_f, EqFunc eq_f,
                      IneqFunc ineq_f) {
        auto start = std::chrono::high_resolution_clock::now();
        ...
        for (size_t i = 0; i < N_vars; ++i) {
            for (size_t j = 0; j < N_vars; ++j) {
                ...
                if (std::abs(val) > 1e-9) {
                    qp_solver_sparse.P.add_value(static_cast<int>(i), static_cast<int>(j), val);
                }
            }  // j 루프 종료
        ...
        for (size_t i = 0; i < N_ineq; ++i) {
            ...
            for (size_t j = 0; j < N_vars; ++j) {
                if (std::abs(J_ineq(i, j)) > 1e-9) {
                    qp_solver_sparse.A_ineq.add_value(static_cast<int>(i), static_cast<int>(j),
                                                      J_ineq(i, j));
                }
            }
        }
