 ...
 * 매 반복마다 2차 계획법(QP) 하위 문제를 생성하고 IPM(Interior Point Method) 솔버를 통해 해를 찾습니다.
 * 
...
public:
     ...
     * @return bool 최적화 성공 여부 (현재 버전에서는 수렴 조건 충족 시 true 반환, 루프 종료 시 false)
     */
    ...
    bool solve(StaticVector<double, N_vars>& u, CostFunc cost_f, EqFunc eq_f, IneqFunc ineq_f,
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            if constexpr (N_eq > 0) {
                StaticVector<double, N_eq> eq_val = eq_f(u); // 현재 위치에서의 등식 제약 조건 값 평가
                StaticMatrix<double, N_eq, N_vars> J_eq = AutoDiff::jacobian<N_eq, N_vars>(eq_f, u); // 야코비안 계산
                for (size_t i = 0; i < N_eq; ++i) {
                    qp_solver.b_eq(static_cast<int>(i)) = -eq_val(static_cast<int>(i)); // 우변: -c(u)
            ...
            if constexpr (N_ineq > 0) {
                StaticVector<double, N_ineq> ineq_val = ineq_f(u); // 현재 위치에서의 부등식 제약 조건 값 평가
                StaticMatrix<double, N_ineq, N_vars> J_ineq =
                    AutoDiff::jacobian<N_ineq, N_vars>(ineq_f, u); // 야코비안 계산
                for (size_t i = 0; i < N_ineq; ++i) {
                    qp_solver.b_ineq(static_cast<int>(i)) = -ineq_val(static_cast<int>(i)); // 우변: -g(u)
                    for (size_t j = 0; j < N_vars; ++j) {
                        qp_solver.A_ineq(static_cast<int>(i), static_cast<int>(j)) =
                            J_ineq(static_cast<int>(i), static_cast<int>(j)); // 좌변: Jacobian 행렬
                    }
                }
            }
            ...
            if (!qp_solver.solve(p, 50, 1e-4)) {
                // QP 풀이에 실패하면, 단순 경사 하강법(Gradient Descent) 방향으로 약간 이동시킵니다.
                p = grad_f * -0.05; 
            }
            ...
            for(size_t i=0; i<N_vars; ++i) {
                ...
                // 한 제어 주기당 변화할 수 있는 물리적 최대 한계치 클리핑 (예: 가속도 +-3.0, 조향 +-3.0 등)
                // 너무 큰 step이 발생하여 시스템이 불안정해지는 것을 막습니다.
            ...
            // 스텝 크기가 1e-6보다 작으면 더 이상 유의미한 변화가 없다고 판단하고 최적해 도달로 간주합니다.
            if (p_norm < 1e-6) return true;

            // 6. 라인 서치 (Line Search) 기반 스텝 크기 결정
            // L1 Merit Function을 사용하여 목적 함수 값의 감소와 제약 조건 위반도 감소의 균형을 맞춥니다.
            ...
            if constexpr (N_ineq > 0) {
                ...
                for (size_t i = 0; i < N_ineq; ++i)
                    current_merit += rho * std::max(0.0, v(static_cast<int>(i))); // 부등식 제약 위반: max(0, v)
            }
            ...
            for (int ls_iter = 0; ls_iter < 10; ++ls_iter) {
                ...
                // Armijo 조건과 유사하게, Merit 함수 값이 감소했으면 적절한 스텝 사이즈로 간주하고 루프 종료
