            ...
            bool solve(StaticVector<double, N_vars>& u, CostFunc cost_f, EqFunc eq_f, IneqFunc ineq_f, int max_iter = 50) {
                for (int iter = 0; iter < max_iter; ++iter) {
                    ...
                    if constexpr (N_ineq > 0) {
                        ...
                        for (size_t i = 0; i < N_ineq; ++i) {
                            current_merit += rho * std::max(0.0, v(static_cast<int>(i)) + alpha * p(static_cast<int>(i)));
                        }
                    }
                    ...
                    for (int ls_iter = 0; ls_iter < 10; ++ls_iter) {
                        ...
                        for (size_t i = 0; i < N_vars; ++i) {
                            u_next(static_cast<int>(i)) = u(static_cast<int>(i)) + alpha * p(static_cast<int>(i));
                        }
                    ...
                    if (s_norm_sq > 1e-12) {
                        ...
                        // Powell's Damping : 제약 조건으로 인해 H가 양의 정부호 (Positivie Definite)를 잃는 것을 방지
                        double theta = 1.0;
                        if (ys < 0.2 * sHs) {
