...
class SecantMethod {
   public:
    ...
    [[nodiscard]] static OptimizationResult1D optimize(Func f, double x0, double x1,
        ...
        for (iter = 1; iter <= max_iter; ++iter) {
            ...
            if (std::abs(diff) < 1e-12) {
                if (verbose) std::cout << "  ↳ ⚠️ Terminated due to zero gradient difference.\n";
                break;
            }
            ...
            if (verbose) {
                std::cout << "[Iter " << std::setw(3) << iter << "] x: " << std::fixed
                          << std::setprecision(6) << x1 << " | f'(x): " << g1
                          << " | |Δ|: " << std::abs(delta) << "\n";
            }
