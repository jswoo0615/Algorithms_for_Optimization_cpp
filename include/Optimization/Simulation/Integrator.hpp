...
class Integrator {
   public:
    ...
    static StaticVector<T, Nx> rk4(const Model& model, const StaticVector<T, Nx>& x,
        ...
        for (size_t i = 0; i < Nx; ++i) {
            x_next(static_cast<int>(i)) =
                x(static_cast<int>(i)) +
                sixth_dt * (k1(static_cast<int>(i)) + two * k2(static_cast<int>(i)) +
                            two * k3(static_cast<int>(i)) + k4(static_cast<int>(i)));
        }
