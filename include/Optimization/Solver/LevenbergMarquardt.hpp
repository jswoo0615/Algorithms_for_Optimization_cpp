...
namespace Optimization {
    namespace solver {
        ...
        SolverStatus solve_LM(StaticVector<double, N>& x_opt, const Functor& calc_residuals, int max_iter = 50, double tol = 1e-6, double initial_lambda = 1e-3) {
            ...
            for (int iter = 0; iter < max_iter; ++iter) {
                ...
                for (size_t i = 0; i < N; ++i) {
                    ...
                    H_lm(static_cast<int>(i), static_cast<int>(i)) += lambda * (H(static_cast<int>(i), static_cast<int>(i)) + 1e-6);
                }
                ...
                double new_cost = 0.0;
                for (size_t k = 0; k < M; ++k) {
                    new_cost += 0.5 * new_residuals(static_cast<int>(k)).v * new_residuals(static_cast<int>(k)).v;
                }
