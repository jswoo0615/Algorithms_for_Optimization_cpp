    ...
    class StrongBacktrackingLineSearch {
        ...
        public: 
            ...
            static double search(Func f, const std::array<double, N>& x, const std::array<double, N>& d, 
                                 double alpha_init=1.0, double c1=1e-4, double c2=0.9, bool verbose=false) {
                // 1. 출발점 정보 획득
                double f_x;
                ...
                while (true) {
                    ...
                    if (verbose) {
                        std::cout << "  ↳ [Wolfe Iter " << std::setw(2) << iter << "] alpha: " << std::fixed << std::setprecision(6) << alpha 
                                << " | Range: [" << alpha_lo << ", " << (alpha_hi > 1e8 ? "INF" : std::to_string(alpha_hi)) << "]\n";
                    }
                    ...
                    if (std::abs(alpha_hi - alpha_lo) < 1e-10) {
                        if (verbose) std::cout << "  ↳ [Failsafe] Bracket too small, returning best alpha.\n";
                        return alpha;
                    }
                }
            }
