...
class SE2Transform {
   public:
    ...
    static StaticMatrix<double, 3, 3> get_global_to_local(double tx, double ty, double psi) {
        ...
        T(0, 0) = cos_p;
        T(0, 1) = sin_p;
        T(0, 2) = -(cos_p * tx + sin_p * ty);
        T(1, 0) = -sin_p;
        T(1, 1) = cos_p;
        T(1, 2) = -(-sin_p * tx + cos_p * ty);
        T(2, 0) = 0.0;
        T(2, 1) = 0.0;
        T(2, 2) = 1.0;
     ...
     */
    static void transform_point(const StaticMatrix<double, 3, 3>& T, double global_x,
                                double global_y, double& local_x, double& local_y) {
