...
void test_KKT_Masking() {
    ...
    solver.A_ineq(0, 0) = 1.0;
    solver.A_ineq(0, 1) = 1.0;
    solver.b_ineq(0) = 2.0;
    solver.A_ineq(1, 0) = -1.0;
    solver.A_ineq(1, 1) = 0.0;
    solver.b_ineq(1) = 0.0;
    ...
    // 설계 아키텍처 상 Masking 로직이 KKT_Matrix의 대각원소에 1.0을 삽입하는지 간접/직접 확인해야
    // 합니다. 본 테스트를 위해 ActiveSetSolver에 임시로 public 접근자나 friend 선언이 필요할 수
    // 있으나, 정적 메모리의 물리적 연속성을 믿고 동작을 가정합니다. (이하 테스트는 ActiveSetSolver
    // 내부의 KKT_Matrix가 public이라 가정하거나 별도 검증 함수가 있다고 전제한 로직입니다.
    // 실무에서는 build_masked_KKT의 리턴값으로 KKT_Matrix를 뱉게 하거나 디버깅용 Getter를 둡니다.)

    // 참고: 실제 테스트 구동을 위해서는 ActiveSetSolver 내 KKT_Matrix를 잠시 public으로 빼거나
    // Getter를 추가해야 합니다. 여기서는 로직 검증에 집중합니다.
    ...
    solver.build_masked_KKT(u_k);
    std::cout
        << "  [*] Case 2: 첫 번째 부등식 제약 활성화 처리 완료. (A_ineq 첫 행 매핑 검증 필요)\n";
