...
int main(int argc, char** argv) {
    ...
    std::cout
        << "  [Layer 6] RT-NMPC (Real-Time Iteration + Hard Constraints) with WCET Profiler\n";
    std::cout << std::string(95, '=') << "\n";
    ...
    std::cout << " Step | Time(s) | Exec Time(s) |    X (m) |    Y (m) | Theta(deg) | Vel(m/s) || "
                 "Accel | Steer(deg)\n";
    ...
    for (int step = 0; step < max_sim_steps; ++step) {
        ...
        std::chrono::duration<double> exec_duration =
            end_time - start_time;  // 초 단위(double) 변환
        ...
        std::cout << std::setw(5) << step << " | " << std::setw(7) << std::fixed
                  << std::setprecision(2) << t << " | " << std::setw(12) << std::fixed
                  << std::setprecision(6) << exec_seconds << " | "  // 6자리 소수점 초 단위 출력
                  << std::setw(8) << std::fixed << std::setprecision(2) << X << " | "
                  << std::setw(8) << Y << " | " << std::setw(10) << theta << " | " << std::setw(8)
                  << v << " || " << std::setw(5) << u_opt(0) << " | " << std::setw(9) << steer_deg
                  << "\n";

        double dist_error = std::sqrt((X - 10.0) * (X - 10.0) + (Y - 10.0) * (Y - 10.0));
        if (dist_error < 0.5 && v < 0.2) {
            std::cout << "\n  [*] RTI Target Reached! (Error: " << dist_error << " m, Vel: " << v
                      << " m/s)\n";
            reached_target = true;
            break;
        }
    ...
    std::cout << std::string(95, '=') << "\n";
    std::cout << "  [Profiler] Worst-Case Execution Time (WCET) : " << std::fixed
              << std::setprecision(6) << max_exec_time << " seconds\n";
    std::cout << "  [Profiler] Control Deadline (dt)            : " << std::fixed
              << std::setprecision(6) << nmpc.dt << " seconds\n";
