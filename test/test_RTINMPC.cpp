...
int main() {
    ...
    StaticVector<double, 6> x_final; x_final.set_zero(); 
    x_final(0) = 10.0; x_final(1) = 10.0; x_final(2) = M_PI/4.0;
    ...
    StaticVector<double, 6> x_curr; x_curr.set_zero(); x_curr(3)=1.0;
    StaticVector<double, Np*2> U_guess; U_guess.set_zero();
    ...
    for (int step = 0; step < 100; ++step) {
        ...
        PathPlanner<Np, 6>::generate_slalom_reference(x_curr, x_final, obs_data, nmpc.dt, ref_horizon);
        ...
        if (std::sqrt(std::pow(x_curr(0)-x_final(0),2)+std::pow(x_curr(1)-x_final(1),2)) < 0.3) {
            std::cout << "\n  [*] Slalom Course Completed!\n";
            break;
        }
        x_curr = Integrator::rk4<6, 2, DynamicBicycleModel, double>(nmpc.model, x_curr, u_opt, nmpc.dt);
