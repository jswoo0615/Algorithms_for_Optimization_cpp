...
int main() {
    ...
    if (!success) {
        std::cout
            << "[ERROR] 수치적 불안정성 발생 (H_uu Matrix is strictly not Positive Definite).\n";
        return -1;
    }
    ...
    for (size_t k = 0; k < Np; ++k) {
        std::cout << std::setw(4) << k << " | " << std::fixed << std::setw(9)
                  << std::setprecision(4) << solver.dx[k](0) << " | " << std::setw(9)
                  << solver.dx[k](1) << " | " << std::setw(13) << solver.du[k](0) << "\n";
    }

    // 터미널 상태 출력
    std::cout << std::setw(4) << Np << " | " << std::fixed << std::setw(9) << std::setprecision(4)
              << solver.dx[Np](0) << " | " << std::setw(9) << solver.dx[Np](1) << " | "
              << std::setw(13) << "-" << "\n";
