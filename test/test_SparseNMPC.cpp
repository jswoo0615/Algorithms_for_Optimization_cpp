...
TEST(SparseNMPC_Test, Frenet_Dynamic_Obstacle_Avoidance) {
    ...
    for (int step = 0; step < 80; ++step) {
        ...
        double steer_deg =
            u_applied(0) *
            (180.0 / M_PI);  // 이제 U(0)가 조향입니다 (기존 코드와 인덱스 일치 여부 확인)

        // 로깅
        std::cout << std::setw(4) << step << " | " << std::fixed << std::setprecision(2)
                  << std::setw(6) << x_true(0) << " | "  // S
                  << std::setw(6) << x_true(1) << " | "  // D
                  << std::setw(6) << x_meas(1) << " | " << std::setw(6) << ekf.x_est(1) << " | "
                  << std::setw(6) << ekf.x_est(2) * (180 / M_PI) << " | " << std::setw(10)
                  << steer_deg << " | " << std::setprecision(4) << std::setw(7) << res.max_kkt_error
                  << "\n";
