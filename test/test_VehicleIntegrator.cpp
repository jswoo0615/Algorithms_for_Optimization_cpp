...
TEST(VehicleIntegratorTest, RK4_AutoDiff_Jacobian) {
    ...
    x(0) = ADVar(0.0); x(1) = ADVar(0.0); x(2) = ADVar(0.0);
    x(3) = ADVar(10.0); x(4) = ADVar(0.0); x(5) = ADVar(0.0);
    ...
    // x_next(5).g[0] 은 "조향각(delta)이 요레이트(r)에 미치는 영향" 즉, 입력 자코비안 B행렬의 (5, 0) 성분입니다.
