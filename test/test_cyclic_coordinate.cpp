...
TEST(CyclicCoordinateTest, BasicSearchConvergesWithGoldenSection) {
    ...
    // GoldenSectionStrategy가 기본값(Default)으로 적용되어 LineSearch::golden_section_search를
    // 호출합니다.
    auto result = CyclicCoordinateSearch::optimize<2>(ConvexQuadratic2D{}, x_init,
                                                      GoldenSectionStrategy{}, 1e-4, 500, false);
