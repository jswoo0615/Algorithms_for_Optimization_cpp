 ...
 *
 * 빌드:
 *   g++ -std=c++17 -O2 -DNDEBUG -o test_static_matrix test_static_matrix.cpp &&
 * ./test_static_matrix
 *
 * 검증 항목:
...
bool mat_near(const StaticMatrix<T, R, C>& A, const StaticMatrix<T, R, C>& B, double tol = TOL) {
...
StaticMatrix<T, N, N> eye() {
    ...
    for (size_t i = 0; i < N; ++i) I(static_cast<int>(i), static_cast<int>(i)) = static_cast<T>(1);
    return I;
}

/** 벡터 두 원소 간 오차 확인 */
template <typename T, size_t N>
bool vec_near(const StaticVector<T, N>& a, const StaticVector<T, N>& b, double tol = TOL) {
...
void test_arithmetic() {
    ...
    auto AB = A * B;
    check(AB(0, 0) == 19 && AB(0, 1) == 22 && AB(1, 0) == 43 && AB(1, 1) == 50,
          "operator*(matrix) 2x2");
...
void test_QR_MGS() {
    ...
    for (int i = 0; i < 2; ++i)
        for (int j = 0; j < 2; ++j) {
            double s = 0.0;
            for (int k = 0; k < 3; ++k) s += Q(k, i) * Q(k, j);
            QtQ(i, j) = s;
        }
    ...
    for (int k = 0; k < 3; ++k) {
        ...
        for (int j = 0; j < 2; ++j) r -= A_orig(k, j) * x(static_cast<size_t>(j));
        res0 += A_orig(k, 0) * r;
        res1 += A_orig(k, 1) * r;
    }
    check(std::abs(res0) < 1e-9 && std::abs(res1) < 1e-9, "QR_MGS 최소제곱 법선방정식 A^T(Ax-b)=0");
...
void test_QR_Householder() {
    ...
    for (int k = 0; k < 4; ++k) {
        double r = b(static_cast<size_t>(k));
        for (int j = 0; j < 3; ++j) r -= A_orig(k, j) * x(static_cast<size_t>(j));
        for (int i = 0; i < 3; ++i) res[i] += A_orig(k, i) * r;
    }
    bool normal_ok = std::abs(res[0]) < 1e-8 && std::abs(res[1]) < 1e-8 && std::abs(res[2]) < 1e-8;
...
void test_edge_cases() {
    ...
    for (int i = 0; i < 8; ++i) Big(i, i) = static_cast<double>(i + 2);  // 대각 우세
